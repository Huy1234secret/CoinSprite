(() => {
  ...
  if (!root) return;
  const view = {
    guildId: '', templates: [], selectedId: '', tab: 'edit', query: '', channelId: '',
    saving: false, saveTimer: null, notice: '', noticeType: '',
  };
  ...
  function newTemplate() {
    ...
    return {
      ...
      content: '',
      containers: [{ id: `container-${stamp}`, accentColor: '#5865F2', text: '## New message\nAdd your message here.', thumbnailUrl: '', imageUrl: '' }],
    };
  }

  function newContainer() {
    return { id: `container-${Date.now().toString(36)}`, accentColor: '#5865F2', text: '## Container title\nWrite your message here.', thumbnailUrl: '', imageUrl: '' };
  }
  ...
  async function saveSelected() {
    ...
    updateSaveState();
    try {
      const payload = await request(`/api/guilds/${view.guildId}/message-templates/${template.id}`, { method: 'PUT', body: JSON.stringify(template) });
  ...
  function renderMarkdown(value) {
    ...
    return String(value || '').split('\n').map((line) => {
      if (line.trim().startsWith('```')) { code = !code; return code ? '<pre><code>' : '</code></pre>'; }
      if (code) return `${escapeHtml(line)}\n`;
      if (line.startsWith('## ')) return `<div class="message-preview-heading">${escapeHtml(line.slice(3))}</div>`;
      if (line.startsWith('-# ')) return `<div class="message-preview-small">${escapeHtml(line.slice(3))}</div>`;
      if (line.startsWith('> ')) return `<div class="message-preview-quote">${escapeHtml(line.slice(2))}</div>`;
      return `<div>${escapeHtml(line).replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`([^`]+)`/g, '<code>$1</code>') || '&nbsp;'}</div>`;
    }).join('');
  }

  function messagePreview(template) {
    return `<div class="message-discord-preview">
      <div class="message-author"><div class="message-bot-avatar">CS</div><div><strong>CoinSprite</strong><span>APP</span></div></div>
      ${template.content ? `<div class="message-root-content">${renderMarkdown(template.content)}</div>` : ''}
      ${template.containers.map((container) => `<div class="message-preview-container" style="--container-color:${escapeHtml(container.accentColor)}">
        ${container.thumbnailUrl ? `<img class="message-preview-thumb" src="${escapeHtml(container.thumbnailUrl)}" alt="">` : ''}
        <div class="message-preview-text">${container.text.split(/<separator>/gi).map((section, index) => `${index ? '<div class="message-preview-separator"></div>' : ''}${renderMarkdown(section.trim())}`).join('')}</div>
  ...
  function renderList() {
    ...
    root.innerHTML = `<div class="message-list-head"><div><h3>Message templates</h3><p>Create reusable Discord messages for this server.</p></div><button class="button primary" type="button" data-message-action="create">Create template</button></div>
      ...
      <div class="message-template-grid">${templates.map((item) => `<button class="message-template-card" type="button" data-message-action="open" data-id="${escapeHtml(item.id)}"><span class="message-template-symbol">✉</span><span><strong>${escapeHtml(item.name)}</strong><small>${item.containers.length} container${item.containers.length === 1 ? '' : 's'}</small></span><span class="message-card-arrow">›</span></button>`).join('')}</div>
  ...
  function renderContainer(container, index) {
    return `<article class="message-container-editor" data-container-index="${index}">
      <div class="message-container-head"><strong>Container ${index + 1}</strong><button class="icon-button danger-text" type="button" data-message-action="remove-container" data-index="${index}" title="Remove container">×</button></div>
      <label>Container color <input type="color" value="${escapeHtml(container.accentColor)}" data-container-field="accentColor"></label>
      <label>Text <textarea rows="9" maxlength="4000" data-container-field="text">${escapeHtml(container.text)}</textarea></label>
      <div class="grid"><label>Thumbnail URL <input type="url" maxlength="1000" value="${escapeHtml(container.thumbnailUrl)}" data-container-field="thumbnailUrl" placeholder="https://..."></label><label>Image URL <input type="url" maxlength="1000" value="${escapeHtml(container.imageUrl)}" data-container-field="imageUrl" placeholder="https://..."></label></div>
      <p class="condition-help">Discord Markdown is supported. Add <code>&lt;separator&gt;</code> where you want a divider.</p>
    </article>`;
  }

  function renderEdit(template) {
    return `<div class="message-edit-layout"><div class="message-edit-fields">
      <div class="panel"><label>Message content <textarea rows="4" maxlength="2000" data-template-field="content" placeholder="Optional text shown above the containers">${escapeHtml(template.content)}</textarea></label></div>
    ...
    </div><div class="message-sticky-preview"><div class="panel-heading"><h3>Live preview</h3><p>Preview your message as it will appear in Discord.</p></div>${messagePreview(template)}</div></div>`;
  }

  function renderUse(template) {
  ...
  function mountUsePicker() {
    ...
    renderPicker(mount, options, view.channelId, { type: 'channel', placeholder: 'Select a channel', onChange: (value) => { view.channelId = value; render(); } });
  }

  function renderEditor() {
    const template = selected();
    if (!template) return renderList();
    root.innerHTML = `<div class="message-editor-head"><button class="icon-button" type="button" data-message-action="back" title="Back">←</button><label>Template name <input type="text" maxlength="80" value="${escapeHtml(template.name)}" data-template-field="name"></label><span id="messageSaveState" class="message-save-state ${view.noticeType}">${escapeHtml(view.notice)}</span><button class="button danger" type="button" data-message-action="delete">Delete</button></div>
      <nav class="message-editor-tabs">${[['edit', 'Edit'], ['preview', 'Preview'], ['use', 'Use']].map(([value, label]) => `<button type="button" class="${view.tab === value ? 'active' : ''}" data-message-action="tab" data-value="${value}">${label}</button>`).join('')}</nav>
      <div class="message-editor-body">${view.tab === 'edit' ? renderEdit(template) : view.tab === 'preview' ? `<div class="message-preview-page">${messagePreview(template)}</div>` : renderUse(template)}</div>`;
    mountUsePicker();
  }
  ...
  root.addEventListener('input', (event) => {
    ...
    if (event.target.dataset.templateField || event.target.dataset.containerField) {
      ...
      if (view.tab === 'edit') {
        ...
        if (preview) preview.innerHTML = `<div class="panel-heading"><h3>Live preview</h3><p>Preview your message as it will appear in Discord.</p></div>${messagePreview(template)}`;
      }
    }
  });
  ...
  root.addEventListener('click', async (event) => {
    ...
    if (action === 'create') {
      ...
      view.templates.push(template); view.selectedId = template.id; view.tab = 'edit'; view.notice = 'Unsaved changes'; view.noticeType = 'pending'; render(); scheduleSave();
    ...
    else if (action === 'tab') { view.tab = button.dataset.value; render(); }
      ...
      view.templates = view.templates.filter((item) => item.id !== template.id); view.selectedId = ''; render();

(() => {
  // admin/client/state.js
  var CARD_UNICODE_FALLBACK = '"Noto Sans SC Variable", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Sans", "DejaVu Sans", sans-serif';
  var CARD_FONT_FAMILY = '"Noto Sans Variable", ' + CARD_UNICODE_FALLBACK;
  var CARD_FONT_FAMILIES = Object.freeze({
  ...
  var CARD_REQUIRED_FONT_FACES = Object.freeze([
  ...
  var EMPTY_EMOJI_DATA = Object.freeze({ version: "", emojiCount: 0, groups: Object.freeze([]) });
  var DEFAULT_EMOJI_DATA_URL = document.querySelector("#emojiDataAsset")?.dataset.src || "/admin/emojiData.js";
  var EMOJI_RENDER_BATCH = 96;
  var EMOJI_SEARCH_DEBOUNCE_MS = 120;
  var DEFAULT_EMOJI_DATA = { value: window.COINSPRITE_EMOJI_DATA || EMPTY_EMOJI_DATA };
  var defaultEmojiDataPromise = { value: null };
  var emojiSearchTimer = { value: null };
  var defaultEmojiItemCache = /* @__PURE__ */ new Map();
  var directoryEmojiItemCache = /* @__PURE__ */ new Map();
  var cardFontLoads = /* @__PURE__ */ new Map();
  var CARD_PREVIEW_DEBOUNCE_MS = 350;
  var CARD_SNAP_DISTANCE = 6;
  var CARD_SNAP_RELEASE = 10;
  var CARD_HISTORY_LIMIT = 60;
  var CARD_TEMPLATES = Object.freeze({
    classic: {
      panelOpacity: 0.85,
      colors: { surface: "#18201b", accent: "#b9f547", text: "#f4f7f2", muted: "#a3ada6", track: "#303a33", progress: "#b9f547" },
      avatar: { x: 54, y: 68, size: 150, color: "#b9f547" },
      username: { x: 236, y: 70, size: 34, color: "#f4f7f2" },
      level: { x: 236, y: 130, size: 24, color: "#b9f547" },
      rank: { x: 934, y: 68, size: 28, color: "#f4f7f2" },
      progress: { x: 236, y: 211, width: 698, height: 27, color: "#b9f547", trackColor: "#303a33" },
      xp: { x: 236, y: 269, size: 21, color: "#d8ded9" }
    },
    arcade: {
      panelOpacity: 0,
      colors: { surface: "#17132d", accent: "#925cff", text: "#ffffff", muted: "#ddd4ff", track: "#46336e", progress: "#c8a8ff" },
      avatar: { x: 410, y: 16, size: 180, color: "#925cff" },
      username: { x: 438, y: 214, size: 36, color: "#ffffff" },
      level: { x: 465, y: 282, size: 22, color: "#ffffff" },
      rank: { x: 970, y: 28, size: 32, color: "#d7b9ff" },
      progress: { x: 345, y: 247, width: 320, height: 25, color: "#c8a8ff", trackColor: "#46336e" },
      xp: { x: 452, y: 249, size: 19, color: "#ffffff" }
    },
    split: {
      panelOpacity: 0.58,
      colors: { surface: "#101d26", accent: "#57d6ff", text: "#f5fcff", muted: "#a9c7d3", track: "#263d49", progress: "#57d6ff" },
      avatar: { x: 70, y: 58, size: 185, color: "#57d6ff" },
      username: { x: 300, y: 68, size: 40, color: "#f5fcff" },
      level: { x: 302, y: 130, size: 23, color: "#57d6ff" },
      rank: { x: 920, y: 68, size: 32, color: "#f5fcff" },
      progress: { x: 302, y: 210, width: 618, height: 30, color: "#57d6ff", trackColor: "#263d49" },
      xp: { x: 302, y: 260, size: 20, color: "#d9f4ff" }
    },
    minimal: {
      panelOpacity: 0.32,
      colors: { surface: "#101410", accent: "#ffffff", text: "#ffffff", muted: "#c7cec8", track: "#303632", progress: "#ffffff" },
      avatar: { x: 72, y: 92, size: 112, color: "#ffffff" },
      username: { x: 220, y: 74, size: 38, color: "#ffffff" },
      level: { x: 222, y: 135, size: 20, color: "#ffffff" },
      rank: { x: 928, y: 74, size: 28, color: "#ffffff" },
      progress: { x: 222, y: 207, width: 706, height: 18, color: "#ffffff", trackColor: "#303632" },
      xp: { x: 222, y: 252, size: 18, color: "#c7cec8" }
    },
    spotlight: {
      panelOpacity: 0.68,
      colors: { surface: "#241711", accent: "#ffad63", text: "#fff8f1", muted: "#e5c8b1", track: "#503526", progress: "#ffad63" },
      avatar: { x: 425, y: 34, size: 150, color: "#ffad63" },
      username: { x: 395, y: 200, size: 36, color: "#fff8f1" },
      level: { x: 444, y: 258, size: 20, color: "#ffcf9f" },
      rank: { x: 930, y: 42, size: 28, color: "#fff8f1" },
      progress: { x: 320, y: 238, width: 360, height: 20, color: "#ffad63", trackColor: "#503526" },
      xp: { x: 425, y: 238, size: 17, color: "#fff8f1" }
    }
  });
  var state = {
    me: null,
    ...
    emojiTarget: null,
    inlineTextCarets: /* @__PURE__ */ new Map(),
    xpDropTesting: false,
    profile: null,
  ...
  var elements = {
    ...
    ownerNav: $("#ownerNav"),
    levelingNav: $("#levelingNav"),
    welcomeMessagesNav: $("#welcomeMessagesNav"),
    messageTemplatesNav: $("#messageTemplatesNav"),
    reactionRolesNav: $("#reactionRolesNav"),
    gamesNav: $("#gamesNav"),
    levelingView: $("#levelingView"),
    welcomeMessagesView: $("#welcomeMessagesView"),
    messageTemplatesView: $("#messageTemplatesView"),
    reactionRolesView: $("#reactionRolesView"),
    gamesView: $("#gamesView"),
    ownerView: $("#ownerView"),
    toast: $("#toast"),
    saveDock: $("#saveDock"),
    ...
    levelingXpMin: $("#levelingXpMin"),
    levelingXpMax: $("#levelingXpMax"),
    levelingCooldown: $("#levelingCooldown"),
    levelingBaseXp: $("#levelingBaseXp"),
    levelingGrowth: $("#levelingGrowth"),
    levelingMaxLevel: $("#levelingMaxLevel"),
    levelingCurvePreview: $("#levelingCurvePreview"),
    countingChannel: $("#countingChannel"),
    gameCommandSettings: $("#gameCommandSettings"),
    gameAddCommandSetting: $("#gameAddCommandSetting"),
    levelingAnnounceEnabled: $("#levelingAnnounceEnabled"),
    levelingAnnounceChannel: $("#levelingAnnounceChannel"),
    levelingChannels: $("#levelingChannels"),
    levelingStackRewards: $("#levelingStackRewards"),
    levelingRewards: $("#levelingRewards"),
    levelingAddReward: $("#levelingAddReward"),
    levelingBoosts: $("#levelingBoosts"),
    levelingAddBoost: $("#levelingAddBoost"),
    levelingContainerAdd: $("#levelingContainerAdd"),
    levelingAdditionalContainerAdd: $("#levelingAdditionalContainerAdd"),
    levelingThumbnailAdd: $("#levelingThumbnailAdd"),
    levelingGalleryAdd: $("#levelingGalleryAdd"),
    levelingVariablesToggle: $("#levelingVariablesToggle"),
    levelingEmojiToggle: $("#levelingEmojiToggle"),
    levelingComposerPanel: $("#levelingComposerPanel"),
    levelingDiscordFrame: $("#levelingDiscordFrame"),
    levelingMessagePreview: $("#levelingMessagePreview"),
    levelingAdditionalContainers: $("#levelingAdditionalContainers"),
    levelingAccentButton: $("#levelingAccentButton"),
    levelingAccentColor: $("#levelingAccentColor"),
    welcomeMessagesEnabled: $("#welcomeMessagesEnabled"),
    welcomeEventEnabled: $("#welcomeEventEnabled"),
    welcomeEventChannel: $("#welcomeEventChannel"),
    welcomeEventReset: $("#welcomeEventReset"),
    welcomeEventStep: $("#welcomeEventStep"),
    welcomeEventTitle: $("#welcomeEventTitle"),
    welcomeEventDescription: $("#welcomeEventDescription"),
    welcomeEventToggleCopy: $("#welcomeEventToggleCopy"),
    welcomePreviewLabel: $("#welcomePreviewLabel"),
    welcomeVariablesToggle: $("#welcomeVariablesToggle"),
    welcomeContainerAdd: $("#welcomeContainerAdd"),
    welcomeAdditionalContainerAdd: $("#welcomeAdditionalContainerAdd"),
    welcomeThumbnailAdd: $("#welcomeThumbnailAdd"),
    welcomeGalleryAdd: $("#welcomeGalleryAdd"),
    welcomeEmojiToggle: $("#welcomeEmojiToggle"),
    welcomeComposerPanel: $("#welcomeComposerPanel"),
    welcomeDiscordFrame: $("#welcomeDiscordFrame"),
    welcomeMessagePreview: $("#welcomeMessagePreview"),
    welcomeAdditionalContainers: $("#welcomeAdditionalContainers"),
    welcomeAccentButton: $("#welcomeAccentButton"),
    welcomeAccentColor: $("#welcomeAccentColor"),
    levelingUseTemplate: $("#levelingUseTemplate"),
    levelingSaveAsTemplate: $("#levelingSaveAsTemplate"),
    welcomeUseTemplate: $("#welcomeUseTemplate"),
    welcomeSaveAsTemplate: $("#welcomeSaveAsTemplate"),
    templateManager: $("#templateManager"),
    templateTotalCount: $("#templateTotalCount"),
    templateFolderCreate: $("#templateFolderCreate"),
    templateFolderList: $("#templateFolderList"),
    templateSearch: $("#templateSearch"),
    templateListCreate: $("#templateListCreate"),
    templateList: $("#templateList"),
    templateCreateButton: $("#templateCreateButton"),
    templateEmptyCreate: $("#templateEmptyCreate"),
    templateEmptyState: $("#templateEmptyState"),
    templateEditor: $("#templateEditor"),
    templateStatusBadge: $("#templateStatusBadge"),
    templateEditorTitle: $("#templateEditorTitle"),
    templateTimestamps: $("#templateTimestamps"),
    templateDuplicateButton: $("#templateDuplicateButton"),
    templateDeleteButton: $("#templateDeleteButton"),
    templateVariablesToggle: $("#templateVariablesToggle"),
    templateContainerAdd: $("#templateContainerAdd"),
    templateAdditionalContainerAdd: $("#templateAdditionalContainerAdd"),
    templateThumbnailAdd: $("#templateThumbnailAdd"),
    templateGalleryAdd: $("#templateGalleryAdd"),
    templateEmojiToggle: $("#templateEmojiToggle"),
    templateComposerPanel: $("#templateComposerPanel"),
    templateDiscordFrame: $("#templateDiscordFrame"),
    templateMessagePreview: $("#templateMessagePreview"),
    templateAdditionalContainers: $("#templateAdditionalContainers"),
    templateControlPreview: $("#templateControlPreview"),
    templateControls: $("#templateControls"),
    templateAddControl: $("#templateAddControl"),
    templateAccentButton: $("#templateAccentButton"),
    templateAccentColor: $("#templateAccentColor"),
    templateCharacterCount: $("#templateCharacterCount"),
    templateJsonEditor: $("#templateJsonEditor"),
    templateJsonError: $("#templateJsonError"),
    templateJsonFormat: $("#templateJsonFormat"),
    templateJsonCopy: $("#templateJsonCopy"),
    templateJsonImport: $("#templateJsonImport"),
    templateResolvedPayload: $("#templateResolvedPayload"),
    templateName: $("#templateName"),
    templateDescription: $("#templateDescription"),
    templateFolderSelect: $("#templateFolderSelect"),
    templateChannel: $("#templateChannel"),
    templateEnabled: $("#templateEnabled"),
    templateVariableReference: $("#templateVariableReference"),
    templateSendHint: $("#templateSendHint"),
    templateSendChannel: $("#templateSendChannel"),
    templateSendTest: $("#templateSendTest"),
    templateSendNow: $("#templateSendNow"),
    templateShareLink: $("#templateShareLink"),
    templateCopyLink: $("#templateCopyLink"),
    templatePickerDialog: $("#templatePickerDialog"),
    templatePickerSearch: $("#templatePickerSearch"),
    templatePickerList: $("#templatePickerList"),
    templateActionDialog: $("#templateActionDialog"),
    templateActionTitle: $("#templateActionTitle"),
    templateActionCopy: $("#templateActionCopy"),
    templateActionTargetLabel: $("#templateActionTargetLabel"),
    templateActionTarget: $("#templateActionTarget"),
    templateActionHelp: $("#templateActionHelp"),
    templateActionSave: $("#templateActionSave"),
    xpDropsEnabled: $("#xpDropsEnabled"),
    xpDropChannel: $("#xpDropChannel"),
    xpDropAdd: $("#xpDropAdd"),
    xpDropList: $("#xpDropList"),
    xpDropVariables: $("#xpDropVariables"),
    xpDropMessagePreview: $("#xpDropMessagePreview"),
    xpDropClaimPreview: $("#xpDropClaimPreview"),
    xpDropEmojiToggle: $("#xpDropEmojiToggle"),
    xpClaimEmojiToggle: $("#xpClaimEmojiToggle"),
    xpDropTestCrate: $("#xpDropTestCrate"),
    xpDropTestChannel: $("#xpDropTestChannel"),
    xpDropTestButton: $("#xpDropTestButton"),
    reactionRoleCreate: $("#reactionRoleCreate"),
    reactionRoleEmptyCreate: $("#reactionRoleEmptyCreate"),
    reactionRoleCount: $("#reactionRoleCount"),
    reactionRoleList: $("#reactionRoleList"),
    reactionRoleEmpty: $("#reactionRoleEmpty"),
    reactionRoleEditor: $("#reactionRoleEditor"),
    reactionRoleStatus: $("#reactionRoleStatus"),
    reactionRoleName: $("#reactionRoleName"),
    reactionRolePublishedState: $("#reactionRolePublishedState"),
    reactionRoleEnabled: $("#reactionRoleEnabled"),
    reactionRoleDuplicate: $("#reactionRoleDuplicate"),
    reactionRoleDelete: $("#reactionRoleDelete"),
    reactionRoleUseTemplate: $("#reactionRoleUseTemplate"),
    reactionRoleEmojiToggle: $("#reactionRoleEmojiToggle"),
    reactionRoleVariablesToggle: $("#reactionRoleVariablesToggle"),
    reactionRoleContainerToggle: $("#reactionRoleContainerToggle"),
    reactionRoleAdditionalContainer: $("#reactionRoleAdditionalContainer"),
    reactionRoleThumbnailToggle: $("#reactionRoleThumbnailToggle"),
    reactionRoleGalleryToggle: $("#reactionRoleGalleryToggle"),
    reactionRoleComposerPanel: $("#reactionRoleComposerPanel"),
    reactionRoleDiscordFrame: $("#reactionRoleDiscordFrame"),
    reactionRoleAccentButton: $("#reactionRoleAccentButton"),
    reactionRoleAccentColor: $("#reactionRoleAccentColor"),
    reactionRoleMessagePreview: $("#reactionRoleMessagePreview"),
    reactionRoleAdditionalContainers: $("#reactionRoleAdditionalContainers"),
    reactionRoleControlPreview: $("#reactionRoleControlPreview"),
    reactionRoleControls: $("#reactionRoleControls"),
    reactionRoleAddControl: $("#reactionRoleAddControl"),
    reactionRoleChannel: $("#reactionRoleChannel"),
    reactionRolePermissionStatus: $("#reactionRolePermissionStatus"),
    reactionRoleFinalPreview: $("#reactionRoleFinalPreview"),
    reactionRoleSaveDraft: $("#reactionRoleSaveDraft"),
    reactionRolePublish: $("#reactionRolePublish"),
    emojiPickerDialog: $("#emojiPickerDialog"),
    emojiPickerClose: $("#emojiPickerClose"),
    emojiPickerSearch: $("#emojiPickerSearch"),
    emojiPickerStatus: $("#emojiPickerStatus"),
    emojiPickerCategories: $("#emojiPickerCategories"),
    emojiPickerGrid: $("#emojiPickerGrid"),
    profileShell: $("#profileShell"),
    profileAvatar: $("#profileAvatar"),
    profileName: $("#profileName"),
    cardCanvas: $("#levelCardDraftCanvas"),
    cardAuthoritativeCanvas: $("#levelCardCanvas"),
    cardCanvasWrap: $("#cardCanvasWrap"),
    cardLayerList: $("#cardLayerList"),
    cardPreviewLabel: $("#cardPreviewLabel"),
    cardInspector: $("#cardInspector"),
    cardInspectorTitle: $("#cardInspectorTitle"),
    cardBackgroundButton: $("#cardBackgroundButton"),
    ...
    cardTemplateSelect: $("#cardTemplateSelect"),
    cardTemplateButton: $("#cardTemplateButton"),
    cardUndoButton: $("#cardUndoButton"),
    cardRedoButton: $("#cardRedoButton"),
    profileSaveDock: $("#profileSaveDock"),
    cardSaveButton: $("#cardSaveButton"),
    cardResetButton: $("#cardResetButton")
  };
  var toastTimer = { value: null };
  var MAX_ADDITIONAL_MESSAGE_CONTAINERS = 2;
  var MEMBER_MESSAGE_DEFAULTS = Object.freeze({
    enabled: true,
  ...
  var XP_DROP_VARIABLES = [
    ["{crate_name}", "Crate name"],
    ["{xp_min}", "Minimum XP"],
    ["{xp_max}", "Maximum XP"],
    ["{xp}", "Claimed XP"],
    ["{claim_limit}", "Total claim slots"],
    ["{claims_left}", "Remaining claims"],
    ["{list_claimed_user}", "Members who claimed"],
    ["{chance}", "Drop chance percent"],
    ["{drop_every}", "Drop interval"],
    ["{despawn_time}", "Despawn interval or never"],
    ["{user}", "Claiming member mention"],
    ["{username}", "Claiming display name"],
    ["{level}", "Member level"],
    ["{total_xp}", "Member total XP"],
    ["{server}", "Server name"],
    ["{channel}", "Drop channel"],
    ["{separator}", "Discord divider"]
  ];
  var LEVELING_VARIABLES = [
    ["{user}", "Mention the member"],
    ["{user_profile}", "Member profile image URL"],
    ["{username}", "Member display name"],
    ["{level}", "New level"],
    ["{next_level}", "Next level"],
    ["{server}", "Server name"],
    ["{bar}", "Live XP progress bar"],
    ["{progress_xp}", "XP earned inside this level"],
    ["{needed_xp}", "XP required for the next level"],
    ["{total_xp}", "Member total XP"],
    ["{separator}", "Insert a Discord divider in the message"]
  ];
  var MEMBER_MESSAGE_META = Object.freeze({
  ...
  var MEMBER_MESSAGE_COMMON_VARIABLES = [
    ["{user}", "Member mention"],
    ...
    ["{channel}", "Selected channel mention"],
    ["{timestamp}", "Event time"],
    ["{separator}", "Discord divider"]
  ];
  var MEMBER_MESSAGE_EVENT_VARIABLES = Object.freeze({
  ...
  var TEMPLATE_LAYOUT_DEFAULTS = Object.freeze({
    container: true,
    accentColor: "#b9f547",
  ...
  var TEMPLATE_ACTION_TYPES = Object.freeze(["send_message", "give_role", "remove_role", "dm_message"]);
  var TEMPLATE_ACTION_LABELS = Object.freeze({
    send_message: "Send ephemeral message",
    give_role: "Give role",
    remove_role: "Remove role",
    dm_message: "DM message"
  });
  var TEMPLATE_CONTROL_DEFAULTS = Object.freeze({
    type: "none",
    buttons: Object.freeze([]),
    dropdowns: Object.freeze([])
  });
  var GENERIC_TEMPLATE_VARIABLES = [
    ["{server}", "Server name"],
    ["{server_icon}", "Server icon URL"],
  ...
  var cardImages = /* @__PURE__ */ new Map();
  var cardFrame = { value: 0 };
  var cardDrawRequest = { value: 0 };
  var cardFontsReadyPromise = { value: null };
  var CARD_BUILTINS = [
    ["background", "🌄", "Background"],
    ["avatar", "🪪", "Discord profile"],
    ["username", "✏️", "Username"],
    ["level", "🏅", "Level"],
    ["rank", "🏆", "Rank"],
  ...
  // admin/client/owner.js
  function formatNumber(value) {
    return new Intl.NumberFormat().format(Number(value) || 0);
  }
  function formatUptime(ms) {
    const totalMinutes = Math.floor((Number(ms) || 0) / 6e4);
  ...
  function guildIcon(guild) {
    return guild.iconURL ? `<img src="${escapeHtml(guild.iconURL)}" alt="">` : `<span class="guild-fallback">${escapeHtml(guild.name.slice(0, 1).toUpperCase())}</span>`;
  }
  ...
  function startOwnerMetricPolling() {
    ...
    state.metricsTimer = window.setInterval(() => sample("metrics", pollOwnerMetrics), 2e3);
  }
  ...
  var pendingSamples = /* @__PURE__ */ new Set();
  var failedSamples = /* @__PURE__ */ new Set();
  async function sample(kind, read) {
    ...
    try {
      await read();
      failedSamples.delete(kind);
    } catch {
      failedSamples.add(kind);
    } finally {
      pendingSamples.delete(kind);
      document.getElementById("ownerLiveStatus").textContent = failedSamples.size ? `Live ${[...failedSamples].join(" and ")} updates interrupted. Retrying automatically; displayed values may be stale.` : "Live updates connected. Metrics refresh every 2 seconds.";
    }
  }
  ...
  function renderWelcomeMessagePreview(renderTools = true) {
    ...
    renderDiscordComposerPreview({
      frame: elements.welcomeDiscordFrame,
      preview: elements.welcomeMessagePreview,
      accentButton: elements.welcomeAccentButton,
      accentInput: elements.welcomeAccentColor,
      containerButton: elements.welcomeContainerAdd,
      layout,
      contentHtml: text,
  ...
  function insertMemberMessageVariable(token) {
    const event = currentMemberMessage();
    if (!event) return;
    let input = elements.welcomeMessagesView.querySelector("[data-inline-message-editor].editing [data-inline-message-input]") || elements.welcomeMessagePreview.querySelector("[data-inline-message-input]");
    if (!input) return;
    ...
    input.value = `${input.value.slice(0, start)}${token}${input.value.slice(end)}`.slice(0, 3e3);
  ...
  async function uploadWelcomeMedia(input) {
    ...
    try {
      const result = await api(`/api/guilds/${state.guildId}/message-media`, {
        method: "POST",
        body: JSON.stringify({ dataUrl: await readMediaFile(file) })
      });
      const containerIndex = Number(input.dataset.additionalContainerIndex);
  ...
  function previewMediaUrl(value) {
    return String(value || "").trim().toLowerCase() === "{user_profile}" ? previewIdentity().user_profile : validHttpUrl(value) ? String(value).trim() : "";
  }
  function discordInlineMarkdown(value) {
    ...
    html = html.replace(/\*\*([^*\n]+)\*\*/g, "<strong>$1</strong>").replace(/__([^_\n]+)__/g, "<u>$1</u>").replace(/~~([^~\n]+)~~/g, "<s>$1</s>").replace(/\|\|([^|\n]+)\|\|/g, '<span class="discord-spoiler">$1</span>').replace(/\*([^*\n]+)\*/g, "<em>$1</em>").replace(/_([^_\n]+)_/g, "<em>$1</em>");
  ...
  function editorInlineMarkdown(value) {
    ...
    html = html.replace(/`([^`\n]+)`/g, (_, content) => stash(`<span class="markdown-syntax">\`</span><code>${content}</code><span class="markdown-syntax">\`</span>`)).replace(/\*\*([^*\n]+)\*\*/g, (_, content) => stash(`<span class="markdown-syntax">**</span><strong>${content}</strong><span class="markdown-syntax">**</span>`)).replace(/__([^_\n]+)__/g, (_, content) => stash(`<span class="markdown-syntax">__</span><u>${content}</u><span class="markdown-syntax">__</span>`)).replace(/~~([^~\n]+)~~/g, (_, content) => stash(`<span class="markdown-syntax">~~</span><s>${content}</s><span class="markdown-syntax">~~</span>`)).replace(/\|\|([^|\n]+)\|\|/g, (_, content) => stash(`<span class="markdown-syntax">||</span><span class="editor-spoiler">${content}</span><span class="markdown-syntax">||</span>`)).replace(/\*([^*\n]+)\*/g, (_, content) => stash(`<span class="markdown-syntax">*</span><em>${content}</em><span class="markdown-syntax">*</span>`)).replace(/_([^_\n]+)_/g, (_, content) => stash(`<span class="markdown-syntax">_</span><em>${content}</em><span class="markdown-syntax">_</span>`)).replace(/\{(?:user|user_profile|username|level|next_level|server|channel|bar|progress_xp|needed_xp|total_xp|crate_name|xp_min|xp_max|xp|claim_limit|claims_left|list_claimed_user|chance|drop_every|despawn_time|separator)\}/gi, (token) => stash(`<span class="editor-token">${token}</span>`));
  ...
  function interpolateTemplate(template, values = {}) {
    return String(template || "").replace(/\{([a-z0-9_]+)\}/gi, (token, key) => Object.prototype.hasOwnProperty.call(values, key) ? String(values[key] ?? "") : token);
  }
  function previewMessageValue(template, extraValues = {}) {
    ...
    return interpolateTemplate(template, {
      ...
      needed_xp: "{needed_xp}",
      total_xp: "{total_xp}",
      crate_name: crate?.name || "Common Crate",
      xp_min: formatNumber(minimum),
      xp_max: formatNumber(maximum),
      xp: "{xp}",
      claim_limit: formatNumber(crate?.claimLimit ?? 3),
      claims_left: "{claims_left}",
      list_claimed_user: "{list_claimed_user}",
      chance: String(crate?.chancePercent ?? 35),
      drop_every: crate?.dropEvery || "30m",
      despawn_time: crate?.despawnAfter || "never",
      channel: "{channel}",
      ...extraValues
    });
  }
  function renderedEditableTemplate(template, previewValues = {}) {
    ...
    return preview.split(/\{separator\}/gi).map((segment, index) => `${index ? '<div class="discord-separator"></div>' : ""}<div class="discord-text">${discordMarkdown(segment)}</div>`).join("");
  }
  function syncInlineEditorVisual(input) {
    ...
    const previewValues = input.dataset.inlineTemplateScope === "memberMessages" ? memberMessagePreviewValues(state.memberMessageEvent) : input.dataset.inlineTemplateScope === "messageTemplate" ? genericTemplatePreviewValues() : {};
  ...
  function finishInlineMessageEdit(editor) {
    ...
    const display = editor.querySelector("[data-inline-message-display]");
    if (input && display) {
      const previewValues = input.dataset.inlineTemplateScope === "memberMessages" ? memberMessagePreviewValues(state.memberMessageEvent) : ["messageTemplate", "reactionRole"].includes(input.dataset.inlineTemplateScope) ? genericTemplatePreviewValues() : {};
      display.innerHTML = `${renderedEditableTemplate(input.value, previewValues)}<span class="inline-edit-badge" aria-hidden="true">EDIT</span>`;
    }
  }
  function inlineTemplateEditor(template, field = "template", scope = "announcements", label = "level-up message", previewValues = {}, maxLength = 3e3, additionalContainerIndex = null) {
  ...
  async function uploadLevelingMedia(input) {
    ...
    try {
      ...
      const containerIndex = Number(input.dataset.additionalContainerIndex);
      const layout = Number.isInteger(containerIndex) ? state.config.leveling.announcements.additionalContainers[containerIndex]?.layout : state.config.leveling.announcements.layout;
      if (!layout) throw new Error("That container no longer exists.");
  ...
  function renderDiscordComposerPreview({ frame, preview, accentButton, accentInput, containerButton, layout, contentHtml, resolveMedia }) {
    ...
    const thumbnail = layout.thumbnailEnabled ? thumbnailUrl ? `<img class="discord-thumbnail" src="${escapeHtml(thumbnailUrl)}" alt="">` : '<div class="discord-thumbnail placeholder">IMG</div>' : "";
  ...
  function renderMessagePreview(renderTools = true) {
    ...
    renderDiscordComposerPreview({
      frame: elements.levelingDiscordFrame,
      preview: elements.levelingMessagePreview,
      accentButton: elements.levelingAccentButton,
      accentInput: elements.levelingAccentColor,
      containerButton: elements.levelingContainerAdd,
      layout,
      contentHtml: text,
      resolveMedia: previewMediaUrl
    });
    renderAdditionalContainerEditors({
  ...
  function defaultEmojiItems(groupId = "") {
    ...
    const items = groups.flatMap((group) => group.emojis.map(([character, name]) => Object.freeze({
      ...
      source: "default",
      searchName: name,
      searchText: `${name} ${character}`.toLowerCase(),
      groupId: group.id
    })));
  ...
  function renderEmojiCategories(search) {
    ...
    for (const group of DEFAULT_EMOJI_DATA.value.groups) {
      ...
      button.type = "button";
      button.dataset.emojiCategory = group.id;
  ...
  function appendEmojiPickerBatch(reset = false) {
    ...
    if (!items.length) {
      ...
      elements.emojiPickerGrid.append(empty);
      return;
    }
    ...
    for (let index = start; index < end; index += 1) {
      ...
      button.dataset.emojiIndex = String(index);
      button.setAttribute("aria-label", emoji.searchName || emoji.name);
      button.title = emoji.searchName || emoji.name;
    ...
    if (end < items.length) {
      ...
      more.textContent = `Showing ${end} of ${items.length}`;
  ...
  function renderEmojiPicker() {
    ...
    elements.emojiPickerStatus.textContent = failed ? `${state.emojiSection === "bot" ? "Bot" : "Group"} emojis could not be loaded. The other sections still work.` : state.emojiSection === "default" ? `${items.length} emoji${items.length === 1 ? "" : "s"}${search ? " found across all categories" : ` · ${activeGroup?.name || "Default Emojis"}`} · Unicode ${DEFAULT_EMOJI_DATA.value.version}` : `${items.length} emoji${items.length === 1 ? "" : "s"}${search ? " found" : ""}`;
  ...
  function applyPickedEmoji(emoji) {
    ...
    if (target.type === "text") {
      ...
      const maximum = Number(input.maxLength) > 0 ? Number(input.maxLength) : 4e3;
  ...
  function clientReactionId(prefix) {
    const random = window.crypto?.randomUUID?.().replace(/-/g, "").slice(0, 24) || `${Date.now().toString(16)}${Math.random().toString(16).slice(2)}`.slice(0, 24);
  ...
  function normalizeReactionRolesClient(value) {
    ...
    const items = (Array.isArray(source.items) ? source.items : []).map((item, itemIndex) => {
      ...
      return {
        ...
        message: {
          content: String(message.content || "## Choose your roles\nUse the controls below to update your server roles.").slice(0, 4e3),
          layout: normalizeTemplateLayoutClient(message.layout),
          additionalContainers: normalizeAdditionalContainersClient(message.additionalContainers, normalizeTemplateLayoutClient, 4e3),
        ...
        publishedMessageId: String(item?.publishedMessageId || ""),
  ...
  function reactionRoleSnapshot() {
    ...
    return draft ? JSON.stringify({
      ...
      dropdown: draft.dropdown,
      channelId: draft.channelId,
      publishedMessageId: draft.publishedMessageId
    }) : "";
  }
  ...
  function renderReactionRoleMessage() {
    const draft = state.reactionRoleDraft;
    if (!draft) return;
    const content = inlineTemplateEditor(draft.message.content, "content", "reactionRole", "Reaction Role message", genericTemplatePreviewValues(), 4e3);
    renderDiscordComposerPreview({
      frame: elements.reactionRoleDiscordFrame,
      preview: elements.reactionRoleMessagePreview,
      accentButton: elements.reactionRoleAccentButton,
      accentInput: elements.reactionRoleAccentColor,
      containerButton: elements.reactionRoleContainerToggle,
      layout: draft.message.layout,
      contentHtml: content,
      resolveMedia: templatePreviewMediaUrl
    });
    renderAdditionalContainerEditors({
      root: elements.reactionRoleAdditionalContainers,
      containers: draft.message.additionalContainers,
      prefix: "reaction",
      scope: "reactionRole",
      previewValues: genericTemplatePreviewValues(),
      resolveMedia: templatePreviewMediaUrl,
      maxLength: 4e3
    });
  ...
  function renderReactionRoleChannel() {
    const draft = state.reactionRoleDraft;
    if (!draft) return;
    elements.reactionRoleChannel.innerHTML = channelOptions(draft.channelId, (channel2) => channel2.sendable === true && channel2.kind !== "forum", "Choose a message channel");
    ...
    const controls = draft.interactionType === "button" ? draft.buttons.map((button) => `<button type="button" class="rr-preview-button ${button.style.toLowerCase()}" disabled>${reactionRoleEmojiHtml(button.emoji)} ${escapeHtml(button.label)}</button>`).join("") : `<div class="rr-preview-select">${escapeHtml(draft.dropdown.placeholder)} · ${draft.dropdown.options.length} option${draft.dropdown.options.length === 1 ? "" : "s"}</div>`;
  ...
  async function selectReactionRole(id) {
    ...
    if (!item) return;
    state.reactionRoleSelectedId = item.id;
    state.reactionRoleDraft = clone(item);
    state.reactionRoleSavedSnapshot = reactionRoleSnapshot();
    state.reactionRoleTab = "message";
    state.reactionRoleComposerPanel = "";
    renderReactionRoles();
  }
  async function createReactionRole() {
    ...
    state.reactionRoleTab = "message";
    elements.reactionRoleName.focus();
    elements.reactionRoleName.select();
    showToast("Reaction Role template created.");
  }
  ...
  async function saveReactionRole() {
    ...
    try {
      ...
      replaceReactionRoles(payload, payload.item.id);
      showToast("Reaction Role draft saved.");
      return payload.item;
    } finally {
      state.reactionRoleSaving = false;
      refreshDirty();
    }
  }
  async function duplicateReactionRole() {
    ...
    replaceReactionRoles(payload, payload.item.id);
    showToast("Reaction Role template duplicated.");
  }
  async function deleteReactionRole() {
    ...
    replaceReactionRoles(payload, "");
    showToast("Reaction Role template deleted.");
  }
  ...
  function updateReactionRoleFromControl(target) {
    ...
    if (target.matches("[data-inline-message-input]")) {
      ...
      if (Number.isInteger(index) && draft.message.additionalContainers[index]) draft.message.additionalContainers[index].content = target.value.slice(0, 4e3);
      else draft.message.content = target.value.slice(0, 4e3);
      syncInlineEditorVisual(target);
      refreshDirty();
      return;
    }
    if (target === elements.reactionRoleName) {
      draft.name = target.value.slice(0, 80);
      renderReactionRoleList();
    }
    if (target === elements.reactionRoleEnabled) draft.enabled = target.checked;
    if (target === elements.reactionRoleAccentColor) {
      draft.message.layout.accentColor = target.value;
      renderReactionRoleMessage();
    }
    if (target === elements.reactionRoleChannel) {
      draft.channelId = target.value;
      renderReactionRoleChannel();
    }
    if (target.matches("[data-reaction-thumbnail-url]")) {
      draft.message.layout.thumbnailUrl = target.value.slice(0, 2e3);
      draft.message.layout.thumbnailEnabled = validTemplateMedia(target.value);
      renderReactionRoleMessage();
    }
    if (target.matches("[data-reaction-gallery-url]")) {
      draft.message.layout.galleryUrls[Number(target.dataset.reactionGalleryUrl)] = target.value.slice(0, 2e3);
      renderReactionRoleMessage();
    }
    if (target.matches("[data-reaction-additional-accent]")) {
      const container = draft.message.additionalContainers[Number(target.dataset.reactionAdditionalAccent)];
    ...
    if (target.matches("[data-reaction-additional-thumbnail-url]")) {
      ...
      if (container) {
        container.layout.thumbnailUrl = target.value.slice(0, 2e3);
    ...
    if (target.matches("[data-reaction-additional-gallery-url]")) {
      ...
      const container = draft.message.additionalContainers[containerIndex];
      if (container) container.layout.galleryUrls[mediaIndex] = target.value.slice(0, 2e3);
      renderReactionRoleMessage();
    }
    if (target.matches("[data-rr-button-label]")) {
      draft.buttons[Number(target.dataset.rrButtonLabel)].label = target.value.slice(0, 80);
      renderReactionRoleControlPreview();
    }
    if (target.matches("[data-rr-button-role]")) draft.buttons[Number(target.dataset.rrButtonRole)].roleId = target.value;
    if (target.matches("[data-rr-button-style]")) {
      draft.buttons[Number(target.dataset.rrButtonStyle)].style = target.value;
      renderReactionRoleControlPreview();
    }
    if (target.matches("[data-rr-dropdown-placeholder]")) {
      draft.dropdown.placeholder = target.value.slice(0, 150);
      renderReactionRoleControlPreview();
    }
    if (target.matches("[data-rr-allow-multiple]")) draft.dropdown.allowMultiple = target.checked;
    if (target.matches("[data-rr-option-title]")) {
      draft.dropdown.options[Number(target.dataset.rrOptionTitle)].title = target.value.slice(0, 100);
      renderReactionRoleControlPreview();
    }
    ...
    elements.reactionRoleStatus.textContent = "UNSAVED";
    elements.reactionRoleStatus.classList.add("unsaved");
    refreshDirty();
  }
  function insertReactionRoleVariable(token) {
    ...
    const start = Number.isInteger(input.selectionStart) ? input.selectionStart : input.value.length;
    const end = Number.isInteger(input.selectionEnd) ? input.selectionEnd : start;
    input.value = `${input.value.slice(0, start)}${token}${input.value.slice(end)}`.slice(0, 4e3);
    input.dispatchEvent(new Event("input", { bubbles: true }));
    input.focus();
  ...
  async function uploadReactionRoleMedia(input) {
    ...
    try {
      ...
      if (!layout) throw new Error("That container no longer exists.");
      if (input.dataset.reactionMediaUpload === "thumbnail") {
        layout.thumbnailUrl = result.url;
        layout.thumbnailEnabled = true;
      } else {
        const index = Number(input.dataset.mediaIndex);
      ...
      renderReactionRoleMessage();
      refreshDirty();
      showToast("Image uploaded. Save the Reaction Role when ready.");
    } catch (error) {
      showToast(error.message || "Image upload failed.", "error");
    } finally {
      label?.classList.remove("uploading");
  ...
  function normalizeTemplateLayoutClient(value) {
    ...
    return {
      ...
      galleryUrls: [...new Set((Array.isArray(source.galleryUrls) ? source.galleryUrls : []).map((url) => String(url).trim()).filter(validTemplateMedia))].slice(0, 10)
    };
  }
  function normalizeTemplateActionClient(value) {
    ...
    return ["give_role", "remove_role"].includes(type) ? { type, roleId: /^\d{16,20}$/.test(String(source.roleId || "")) ? String(source.roleId) : "" } : { type, templateId: /^[a-zA-Z0-9_-]{8,64}$/.test(String(source.templateId || "")) ? String(source.templateId) : "" };
  }
  ...
  function normalizeMessageTemplatesClient(value) {
    ...
    const folders = (Array.isArray(source.folders) ? source.folders : []).map((folder) => ({
      id: String(folder.id || ""),
      name: String(folder.name || "Folder").trim().slice(0, 80),
    ...
    const items = (Array.isArray(source.items) ? source.items : []).map((item) => ({
      id: String(item.id || ""),
      folderId: folderIds.has(String(item.folderId || "")) ? String(item.folderId) : null,
      name: String(item.name || "Template").trim().slice(0, 80),
      description: String(item.description || "").trim().slice(0, 500),
      version: 3,
      content: String(item.content || "").slice(0, 4e3),
      layout: normalizeTemplateLayoutClient(item.layout),
      additionalContainers: normalizeAdditionalContainersClient(item.additionalContainers, normalizeTemplateLayoutClient, 4e3),
      controls: Number(item.version) === 1 ? clone(TEMPLATE_CONTROL_DEFAULTS) : Number(item.version) === 2 ? normalizeTemplateControlsV2Client(item.controls) : normalizeTemplateControlsClient(item.controls),
      defaultChannelId: String(item.defaultChannelId || ""),
      enabled: item.enabled !== false,
      createdAt: String(item.createdAt || ""),
      updatedAt: String(item.updatedAt || "")
    })).filter((item) => item.id);
  ...
  function templateDocument(draft = state.templateDraft) {
    return {
      version: 3,
      content: String(draft?.content || "").slice(0, 4e3),
      layout: normalizeTemplateLayoutClient(draft?.layout),
      additionalContainers: normalizeAdditionalContainersClient(draft?.additionalContainers, normalizeTemplateLayoutClient, 4e3),
  ...
  function parseTemplateControlsClient(value) {
    ...
    const buttonIds = /* @__PURE__ */ new Set();
  ...
  function parseTemplateJsonText(text) {
    ...
    const unknown = Object.keys(parsed).filter((key) => !(version === 1 ? ["version", "content", "layout", "additionalContainers"] : ["version", "content", "layout", "additionalContainers", "controls"]).includes(key));
    ...
    if (parsed.content.length > 4e3) throw new Error("content must be 4000 characters or fewer.");
    ...
    const additional = parsed.additionalContainers === void 0 ? [] : parsed.additionalContainers;
    ...
    const additionalContainers = additional.map((container, index) => {
      ...
      if (container.content.length > 4e3) throw new Error(`Additional container ${index + 1} content must be 4000 characters or fewer.`);
    ...
    const controls = version === 1 ? clone(TEMPLATE_CONTROL_DEFAULTS) : version === 2 ? parseTemplateControlsV2Client(parsed.controls) : parseTemplateControlsClient(parsed.controls);
  ...
  function templateSnapshot() {
    return state.templateDraft ? JSON.stringify({
      ...
      folderId: state.templateDraft.folderId,
      defaultChannelId: state.templateDraft.defaultChannelId,
      enabled: state.templateDraft.enabled,
      ...templateDocument()
    }) : "";
  }
  ...
  function newTemplateOption(index = 0, title = `Option ${index + 1}`) {
    return {
      id: clientReactionId("control"),
      emoji: { id: "", name: "✨", animated: false, source: "default" },
      title,
      description: "",
      sortOrder: index,
      action: { type: "send_message", templateId: "" }
    };
  }
  function newTemplateDropdown(index = 0) {
    return {
      id: clientReactionId("dropdown"),
      placeholder: `Choose an option${index ? ` ${index + 1}` : ""}`,
      allowMultiple: false,
      sortOrder: index,
      options: [newTemplateOption()]
    };
  }
  ...
  function refreshTemplateControlValidation() {
    ...
    for (const card of elements.templateControls.querySelectorAll("[data-template-dropdown-card]")) {
      ...
      if (message) {
        message.textContent = cardError;
        message.hidden = !cardError;
      }
  ...
  function renderTemplateControlPreview() {
    ...
    elements.templateControlPreview.innerHTML = draft.controls.type === "button" ? draft.controls.buttons.map((button) => `<button type="button" class="rr-preview-button ${button.style.toLowerCase()}" disabled>${reactionRoleEmojiHtml(button.emoji)} ${escapeHtml(button.label)}</button>`).join("") : "";
  }
  ...
  function resolvedTemplatePayloadPreview() {
    ...
    const buildContainer = (content, layout, forceContainer = false) => {
      ...
      return layout.container || forceContainer ? [{ type: 17, accent_color: Number.parseInt(layout.accentColor.slice(1), 16), components: inner }] : inner;
    };
    ...
    if (state.templateDraft.controls.type === "button") {
      const buttons = state.templateDraft.controls.buttons.map((button) => {
        ...
        const emoji = payloadEmoji(button.emoji);
    ...
    if (state.templateDraft.controls.type === "dropdown") {
      for (const dropdown of state.templateDraft.controls.dropdowns) {
        ...
        components.push({ type: 1, components: [{
          type: 3,
          custom_id: previewId("d", dropdown.id),
          placeholder: dropdown.placeholder,
          min_values: 1,
          max_values: dropdown.allowMultiple ? dropdown.options.length : 1,
          options: dropdown.options.map((option) => {
            ...
            const emoji = payloadEmoji(option.emoji);
  ...
  function renderTemplateVariableReference() {
    ...
    for (const name of used.filter((name2) => name2 !== "separator" && !generic.has(name2))) chips.push(`<span class="template-variable-chip context"><code>{${escapeHtml(name)}}</code><small>Context-specific</small></span>`);
    elements.templateVariableReference.innerHTML = chips.join("");
    const unresolved = used.filter((name) => name !== "separator" && !generic.has(name));
    elements.templateSendHint.textContent = unresolved.length ? `Direct sending is blocked until these context variables are removed: ${unresolved.map((name) => `{${name}}`).join(", ")}` : "Test and live sends use the same visible content. Role/user pings stay disabled.";
    return unresolved;
  }
  ...
  function renderTemplateComposerPreview(renderTools = true, updateJson = true) {
    const draft = state.templateDraft;
    if (!draft) return;
    const contentHtml = inlineTemplateEditor(draft.content, "content", "messageTemplate", "message template", genericTemplatePreviewValues(), 4e3);
    renderDiscordComposerPreview({
      frame: elements.templateDiscordFrame,
      preview: elements.templateMessagePreview,
      accentButton: elements.templateAccentButton,
      accentInput: elements.templateAccentColor,
      containerButton: elements.templateContainerAdd,
      layout: draft.layout,
      contentHtml,
      resolveMedia: templatePreviewMediaUrl
    });
  ...
  async function createMessageTemplate() {
    ...
    const payload = await api(`/api/guilds/${state.guildId}/message-templates`, {
      method: "POST",
      body: JSON.stringify({ name, folderId, content: "", layout: clone(TEMPLATE_LAYOUT_DEFAULTS) })
    });
  ...
  function insertTemplateVariable(token) {
    if (!state.templateDraft) return;
    let input = elements.messageTemplatesView.querySelector("[data-inline-message-editor].editing [data-inline-message-input]") || elements.templateMessagePreview.querySelector("[data-inline-message-input]");
    if (!input) return;
  ...
  async function sendCurrentTemplate(mode) {
    ...
    try {
      const result = await api(`/api/guilds/${state.guildId}/message-templates/${state.templateDraft.id}/send`, {
        method: "POST",
        body: JSON.stringify({ mode, channelId, confirm: mode === "send" })
      });
      showToast(`${mode === "test" ? "Test message" : "Message"} sent to #${result.channelName}.`, "", result.messageUrl);
    } catch (error) {
      showToast(error.message || "The message could not be sent.", "error");
    }
  }
  ...
  function updateTemplateDraftFromControl(target) {
    ...
    if (target.matches("[data-inline-message-input]") && target.dataset.inlineTemplateScope === "messageTemplate") {
      ...
      if (Number.isInteger(containerIndex) && draft.additionalContainers[containerIndex]) draft.additionalContainers[containerIndex].content = target.value.slice(0, 4e3);
      else draft.content = target.value.slice(0, 4e3);
      syncInlineEditorVisual(target);
    ...
    if (target.matches("[data-template-thumbnail-url]")) {
      draft.layout.thumbnailUrl = target.value.slice(0, 2e3);
    ...
    if (target.matches("[data-template-gallery-url]")) {
      draft.layout.galleryUrls[Number(target.dataset.templateGalleryUrl)] = target.value.slice(0, 2e3);
      renderTemplateComposerPreview(false);
      return;
    }
  ...
  function templateControlAt(spec) {
    ...
    if (kind === "button") {
      const entries2 = state.templateDraft?.controls?.buttons || [];
      const index2 = entries2.findIndex((entry) => entry.id === firstId);
      return index2 >= 0 ? { kind, dropdownIndex: null, index: index2, entry: entries2[index2], entries: entries2, spec: `button:${firstId}` } : null;
    }
    ...
    return index >= 0 ? {
      kind,
      dropdown: resolved.dropdown,
      dropdownIndex: resolved.dropdownIndex,
      index,
      entry: entries[index],
      entries,
      spec: `option:${firstId}:${secondId}`
    } : null;
  }
  ...
  function openTemplateActionDialog(spec) {
    ...
    elements.templateActionCopy.textContent = templateAction ? "Choose a Message Template from this server. It is resolved again when the member interacts." : "Choose a safe Discord role below CoinSprite. Role safety is rechecked at interaction time.";
  ...
  async function applyTemplateSnapshot(templateId) {
    ...
    if (state.templatePickerContext === "leveling") {
      state.config.leveling.announcements.template = item.content.slice(0, 3e3);
      ...
      state.config.leveling.announcements.additionalContainers = clone(item.additionalContainers).map((container) => ({
        ...container,
        content: container.content.slice(0, 3e3)
      }));
      renderMessagePreview();
      ...
      state.reactionRoleDraft.message = {
        ...
        layout: clone(item.layout),
        additionalContainers: clone(item.additionalContainers),
        sourceTemplateId: item.id
      };
      renderReactionRoleEditor();
    } else {
      const event = currentMemberMessage();
      event.template = item.content.slice(0, 3e3);
  ...
  async function saveComposerAsTemplate(context) {
    ...
    const payload = await api(`/api/guilds/${state.guildId}/message-templates`, {
      method: "POST",
      body: JSON.stringify({ name, content: defaults.template, layout: defaults.layout, additionalContainers: defaults.additionalContainers })
    });
  ...
  function pushCardHistory(stack, snapshot2) {
    if (!snapshot2 || stack.at(-1) === snapshot2) return;
  ...
  function restoreCardHistory(snapshot2) {
    state.profile.design = JSON.parse(snapshot2);
  ...
  function undoCardDesign() {
    ...
    const snapshot2 = state.cardUndoStack.pop();
    if (!snapshot2) return;
    pushCardHistory(state.cardRedoStack, cardSnapshot());
    restoreCardHistory(snapshot2);
  }
  function redoCardDesign() {
    finishCardInputHistory();
    const snapshot2 = state.cardRedoStack.pop();
    if (!snapshot2) return;
      ...
      for (const face of CARD_REQUIRED_FONT_FACES) {
        ...
        for (const style of styles) {
          for (const weight of [400, 700]) {
            ...
            requests.push(document.fonts.load(font, face.family === "Noto Sans SC Variable" ? "汉字" : "CoinSprite").then((loaded) => {
  ...
  function loadCardFont(item, text) {
    ...
    const sample2 = String(text || "CoinSprite");
    const key = `${font}\0${sample2}`;
    if (cardFontLoads.has(key)) return cardFontLoads.get(key);
    const loading = ensureCardFontsReady().then(() => document.fonts.load(font, sample2)).catch((error) => {
  ...
  function cardTextValue(selection, layer = cardLayerBySelection(selection)) {
    ...
    if (selection === "xp") return preview.neededXp ? `${formatNumber(preview.progressXp)} / ${formatNumber(preview.neededXp)} XP` : `${formatNumber(preview.xp)} XP - MAX LEVEL`;
    return "";
  }
  ...
  function cardPointInBounds(point, bounds) {
    ...
    return local.x >= bounds.x - 5 && local.x <= bounds.x + bounds.width + 5 && local.y >= bounds.y - 5 && local.y <= bounds.y + bounds.height + 5;
  }
  ...
  function cardBounds(selection = state.cardSelection, context = elements.cardCanvas.getContext("2d")) {
    ...
    if (selection === "background") return { x: 0, y: 0, width: 1e3, height: 320, resize: false };
  ...
  function cardHandleMetrics(bounds) {
    ...
    const size = Math.min(10, Math.max(4, shortest * 0.22));
    return {
      size,
      hitRadius: Math.max(5, size),
      rotateDistance: Math.min(28, Math.max(14, shortest * 0.45))
    };
  }
  ...
  function drawCardPreview() {
    ...
    context.save();
    try {
      cardRoundRect(context, 0, 0, 1e3, 320, 30);
      context.clip();
      context.fillStyle = design.background.color;
      context.fillRect(0, 0, 1e3, 320);
      const background = cardImage(design.background.imageUrl);
      if (background) drawCardCover(context, background, 0, 0, 1e3, 320, design.background.x, design.background.y, design.background.scale);
      context.globalAlpha = Number.isFinite(Number(design.panelOpacity)) ? Math.max(0, Math.min(1, Number(design.panelOpacity))) : 0.85;
    ...
    if (Number.isFinite(state.cardGuides.x) || Number.isFinite(state.cardGuides.y)) {
      ...
      if (Number.isFinite(state.cardGuides.x)) {
        context.beginPath();
        context.moveTo(state.cardGuides.x, 0);
        context.lineTo(state.cardGuides.x, 320);
        context.stroke();
      }
      if (Number.isFinite(state.cardGuides.y)) {
        context.beginPath();
        context.moveTo(0, state.cardGuides.y);
        context.lineTo(1e3, state.cardGuides.y);
        context.stroke();
      }
      context.restore();
    }
    ...
    if (bounds) {
      ...
      if (bounds.resize) {
        ...
        context.beginPath();
        context.arc(rotate.x, rotate.y, Math.max(3, controls.size * 0.6), 0, Math.PI * 2);
  ...
  function inspectorInput(label, path, value, options = {}) {
    ...
    const attributes = type === "number" ? ` min="${options.min ?? -1e3}" max="${options.max ?? 1e3}" step="${options.step ?? 1}"` : "";
  ...
  function renderCardInspector() {
    ...
    if (selection === "background") {
      fields = inspectorInput("Card color", "background.color", item.color, { type: "color" }) + inspectorInput("Panel color", "colors.surface", state.profile.design.colors.surface, { type: "color" }) + inspectorInput("Panel opacity", "panelOpacity", state.profile.design.panelOpacity ?? 0.85, { min: 0, max: 1, step: 0.05, wide: true }) + inspectorInput("Image X", "background.x", item.x, { min: -1e3, max: 1e3 }) + inspectorInput("Image Y", "background.y", item.y, { min: -320, max: 320 }) + inspectorInput("Image scale", "background.scale", item.scale, { min: 0.25, max: 5, step: 0.05, wide: true });
    } else if (selection === "avatar") {
      fields = inspectorInput("X", "avatar.x", item.x, { min: 0, max: 950 }) + inspectorInput("Y", "avatar.y", item.y, { min: 0, max: 270 }) + inspectorInput("Size", "avatar.size", item.size, { min: 32, max: 240 }) + inspectorInput("Rotation", "avatar.rotation", item.rotation || 0, { min: -180, max: 180 }) + inspectorInput("Ring color", "avatar.color", item.color, { type: "color", wide: true });
    } else if (selection === "progress") {
      fields = inspectorInput("X", "progress.x", item.x) + inspectorInput("Y", "progress.y", item.y) + inspectorInput("Width", "progress.width", item.width, { min: 40, max: 950 }) + inspectorInput("Height", "progress.height", item.height, { min: 6, max: 70 }) + inspectorInput("Rotation", "progress.rotation", item.rotation || 0, { min: -180, max: 180, wide: true }) + inspectorInput("Bar color", "progress.color", item.color, { type: "color" }) + inspectorInput("Track color", "progress.trackColor", item.trackColor, { type: "color" });
    } else if (layer?.type === "image") {
      fields = inspectorInput("X", `layers.${layer.id}.x`, layer.x) + inspectorInput("Y", `layers.${layer.id}.y`, layer.y) + inspectorInput("Width", `layers.${layer.id}.width`, layer.width, { min: 12, max: 800 }) + inspectorInput("Height", `layers.${layer.id}.height`, layer.height, { min: 12, max: 320 }) + inspectorInput("Rotation", `layers.${layer.id}.rotation`, layer.rotation || 0, { min: -180, max: 180, wide: true }) + '<div class="inspector-divider"></div><button class="inspector-delete" type="button" data-delete-card-layer>Delete image</button>';
    } else if (layer?.type === "text") {
      const prefix = `layers.${layer.id}`;
      fields = inspectorInput("Text", `layers.${layer.id}.text`, layer.text, { type: "textarea", wide: true }) + inspectorInput("X", `layers.${layer.id}.x`, layer.x) + inspectorInput("Y", `layers.${layer.id}.y`, layer.y) + inspectorInput("Font size", `${prefix}.size`, layer.size, { min: 10, max: 96 }) + inspectorInput("Rotation", `${prefix}.rotation`, layer.rotation || 0, { min: -180, max: 180 }) + inspectorSelect("Font", `${prefix}.fontFamily`, layer.fontFamily || "sans") + inspectorFormatting(prefix, layer) + inspectorInput("Color", `${prefix}.color`, layer.color, { type: "color", wide: true }) + '<div class="inspector-divider"></div><button class="inspector-delete" type="button" data-delete-card-layer>Delete text</button>';
    } else {
      const prefix = selection;
      fields = inspectorInput("X", `${selection}.x`, item.x) + inspectorInput("Y", `${selection}.y`, item.y) + inspectorInput("Font size", `${selection}.size`, item.size, { min: 12, max: 80 }) + inspectorInput("Rotation", `${selection}.rotation`, item.rotation || 0, { min: -180, max: 180 }) + inspectorSelect("Font", `${prefix}.fontFamily`, item.fontFamily || "sans") + inspectorFormatting(prefix, item) + inspectorInput("Color", `${selection}.color`, item.color, { type: "color", wide: true });
    }
  ...
  function constrainCardSelection(selection = state.cardSelection) {
    ...
    if (selection === "background") {
      state.profile.design.panelOpacity = limit(state.profile.design.panelOpacity ?? 0.85, 0, 1);
      state.profile.design.background.x = limit(state.profile.design.background.x, -1e3, 1e3);
      state.profile.design.background.y = limit(state.profile.design.background.y, -320, 320);
      state.profile.design.background.scale = limit(state.profile.design.background.scale, 0.25, 5);
      return;
    }
    ...
    if (selection === "avatar") {
      ...
      target.x = limit(target.x, 0, 1e3 - target.size);
      target.y = limit(target.y, 0, 320 - target.size);
      return;
    }
    if (selection === "progress") {
      target.width = limit(target.width, 40, 1e3);
      target.height = limit(target.height, 6, 70);
      target.x = limit(target.x, 0, 1e3 - target.width);
    ...
    if (layer) {
      if (layer.type === "text") {
        ...
        layer.width = Math.min(1e3, Math.max(1, Math.ceil(metrics.width)));
        ...
        layer.width = limit(layer.width, 12, 1e3);
        layer.height = limit(layer.height, 12, 320);
      }
      layer.x = limit(layer.x, 0, 1e3 - layer.width);
    ...
    if (fitted.x + fitted.width > 1e3) target.x -= fitted.x + fitted.width - 1e3;
  ...
  function showAuthoritativeCardPreview(draft) {
    ...
    elements.cardPreviewLabel.querySelector("small").textContent = draft ? "Rendered on the server; save to publish this design to /level." : "This saved PNG is rendered by the same server used by /level.";
  }
  async function loadAuthoritativeCardPreview() {
    ...
    try {
      ...
      try {
        if (request !== state.cardPreviewRequest || expectedSavedHash !== state.profile?.designHash || designSnapshot !== cardSnapshot()) return;
        if (image.width !== 1e3 || image.height !== 320) throw new Error("Authoritative level-card render returned unexpected dimensions.");
        const context = elements.cardAuthoritativeCanvas.getContext("2d");
        context.clearRect(0, 0, 1e3, 320);
  ...
  function canvasPoint(event) {
    ...
    return { x: (event.clientX - box.left) * 1e3 / box.width, y: (event.clientY - box.top) * 320 / box.height };
  }
  ...
  function cardAlignmentTargets(excludedSelection) {
    const x = [{ value: 0, kind: "start" }, { value: 28, kind: "start" }, { value: 500, kind: "center" }, { value: 972, kind: "end" }, { value: 1e3, kind: "end" }];
  ...
  function resizeCardTarget(drag, point) {
    ...
    if (originalBounds.resize === "text") {
      ...
      const scale = Math.max(0.1, ...scales);
  ...
  function moveCardPointer(event) {
    ...
    if (drag.mode === "background") {
      if (target.imageUrl) {
        target.x = Math.round(drag.original.x + dx);
        target.y = Math.round(drag.original.y + dy);
      }
  ...
  // admin/client/navigation.js
  function workspaceStatus(message = "", kind = "", retry = false) {
  ...
  async function api(path, options = {}) {
    ...
    try {
      const response = await fetch(path, {
        ...
        credentials: "same-origin",
        cache: "no-store",
        signal: options.signal || AbortSignal.timeout(45e3)
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
      ...
      connection.dataset.state = "error";
      throw new Error(error.name === "TimeoutError" ? "The request timed out. Check the current state before retrying a send or publish." : "Unable to reach CoinSprite. Check your connection and try again.");
    } finally {
      if (mutation) {
        state.pendingWrites--;
  ...
  function setView(view) {
    ...
    document.querySelectorAll("[data-view]").forEach((button) => {
      ...
      if (active) {
        ...
        document.getElementById("currentSection").textContent = button.querySelector("strong").textContent;
  ...
  function normalizeAdditionalContainersClient(value, normalizeLayout, maximumContent = 3e3) {
  ...
  function normalizeLevelingConfig(config) {
    ...
    source.xp ||= {};
    source.xp.min = Math.round(clampNumber(source.xp.min, 1, 1e3, 15));
    source.xp.max = Math.round(clampNumber(source.xp.max, source.xp.min, 2e3, 25));
    source.xp.cooldownSeconds = Math.round(clampNumber(source.xp.cooldownSeconds, 0, 3600, 60));
    source.curve ||= {};
    source.curve.baseXp = Math.round(clampNumber(source.curve.baseXp, 25, 1e5, 100));
    source.curve.growth = clampNumber(source.curve.growth, 1, 3, 1.5);
    source.curve.maxLevel = Math.round(clampNumber(source.curve.maxLevel, 1, 1e3, 100));
...
${String(source.announcements.progress || "`{bar}` {progress_xp} / {needed_xp} XP toward level {next_level}")}`;
    source.announcements.template = String(source.announcements.template || legacyTemplate).slice(0, 3e3);
    ...
    source.announcements.layout.accentColor = /^#[0-9a-f]{6}$/i.test(source.announcements.layout.accentColor || "") ? source.announcements.layout.accentColor.toLowerCase() : "#b9f547";
    source.announcements.layout.thumbnailEnabled = source.announcements.layout.thumbnailEnabled === true;
    source.announcements.layout.thumbnailUrl = String(source.announcements.layout.thumbnailUrl || "").slice(0, 2e3);
    ...
    source.announcements.additionalContainers = normalizeAdditionalContainersClient(source.announcements.additionalContainers, (layout) => {
      ...
      return {
        ...
        galleryUrls: [...new Set((Array.isArray(normalized.galleryUrls) ? normalized.galleryUrls : []).map((url) => String(url).trim()).filter(validMediaTemplate))].slice(0, 10)
      };
    });
    source.channelMultipliers = Object.fromEntries(Object.entries(source.channelMultipliers || {}).map(([id, multiplier]) => [
      String(id),
      Math.round(clampNumber(multiplier, 0, 10, 1))
    ]));
    ...
    source.xpDrops.crates = (source.xpDrops.crates || []).map((crate, index) => {
      ...
      usedCrateIds.add(id);
      const minimum = Math.round(clampNumber(crate.xp?.min ?? crate.xpMin, 1, 1e6, 50));
      return {
        id,
        enabled: crate.enabled !== false,
        name: String(crate.name || `Crate ${index + 1}`).trim().slice(0, 80) || `Crate ${index + 1}`,
        imageUrl: String(crate.imageUrl || "").slice(0, 2e3),
        xp: { min: minimum, max: Math.round(clampNumber(crate.xp?.max ?? crate.xpMax, minimum, 1e6, Math.max(100, minimum))) },
        ...
        claimLimit: Math.round(clampNumber(crate.claimLimit, 1, 1e3, 1)),
  ...
  function normalizeMemberMessagesConfig(config) {
    ...
    for (const type of ["join", "leave", "boost"]) {
      ...
      event.template = String(event.template || defaults.template).trim().slice(0, 3e3) || defaults.template;
      ...
      event.layout.galleryUrls = [...new Set((Array.isArray(event.layout.galleryUrls) ? event.layout.galleryUrls : []).map((url) => String(url).trim()).filter(validMemberMediaTemplate))].slice(0, 10);
  ...
  function renderXpDrops() {
    ...
    renderXpDropList();
    elements.xpDropTestCrate.innerHTML = xpDrops.crates.length ? xpDrops.crates.map((crate) => `<option value="${escapeHtml(crate.id)}" ${crate.id === selectedCrate ? "selected" : ""}>${escapeHtml(crate.name)}</option>`).join("") : '<option value="">Add a crate first</option>';
  ...
  function updateLevelingFromControl(target) {
    ...
    if (target.matches("[data-inline-message-input]")) {
      ...
      const limits = { template: 3e3 };
      if (target.dataset.inlineTemplateScope === "xpDrops") {
        if (["dropTemplate", "claimTemplate"].includes(field)) leveling.xpDrops[field] = target.value.slice(0, 3e3);
      } else {
        const containerIndex = Number(target.dataset.additionalContainerIndex);
        if (Number.isInteger(containerIndex) && leveling.announcements.additionalContainers[containerIndex]) {
          leveling.announcements.additionalContainers[containerIndex].content = target.value.slice(0, 3e3);
    ...
    if (target === elements.levelingXpMin) {
      leveling.xp.min = Math.round(clampNumber(target.value, 1, 1e3, 15));
      leveling.xp.max = Math.max(leveling.xp.min, leveling.xp.max);
      elements.levelingXpMax.value = leveling.xp.max;
    }
    if (target === elements.levelingXpMax) leveling.xp.max = Math.round(clampNumber(target.value, leveling.xp.min, 2e3, 25));
    if (target === elements.levelingCooldown) leveling.xp.cooldownSeconds = Math.round(clampNumber(target.value, 0, 3600, 60));
    if (target === elements.levelingBaseXp) leveling.curve.baseXp = Math.round(clampNumber(target.value, 25, 1e5, 100));
    ...
    if (target === elements.levelingMaxLevel) {
      leveling.curve.maxLevel = Math.round(clampNumber(target.value, 1, 1e3, 100));
    ...
    if (target.matches("[data-leveling-thumbnail-url]")) {
      leveling.announcements.layout.thumbnailUrl = target.value.slice(0, 2e3);
    ...
    if (target.matches("[data-leveling-gallery-url]")) {
      leveling.announcements.layout.galleryUrls[Number(target.dataset.levelingGalleryUrl)] = target.value.slice(0, 2e3);
      renderMessagePreview(false);
    }
    ...
    if (target.matches("[data-xp-drop-field]")) {
      ...
      if (crate) {
        if (field === "enabled" || field === "allowMultipleClaims") crate[field] = target.checked;
          ...
          crate.xp.min = Math.round(clampNumber(target.value, 1, 1e6, crate.xp.min));
          crate.xp.max = Math.max(crate.xp.min, crate.xp.max);
        } else if (field === "xpMax") crate.xp.max = Math.round(clampNumber(target.value, crate.xp.min, 1e6, crate.xp.max));
        ...
        else if (field === "claimLimit") crate.claimLimit = Math.round(clampNumber(target.value, 1, 1e3, crate.claimLimit));
  ...
  function updateMemberMessagesFromControl(target) {
    ...
    if (target.matches("[data-inline-message-input]")) {
      const containerIndex = Number(target.dataset.additionalContainerIndex);
      if (Number.isInteger(containerIndex) && event.additionalContainers[containerIndex]) event.additionalContainers[containerIndex].content = target.value.slice(0, 3e3);
      else event.template = target.value.slice(0, 3e3);
      syncInlineEditorVisual(target);
      refreshDirty();
      return;
    }
    ...
    if (target.matches("[data-welcome-thumbnail-url]")) {
      event.layout.thumbnailUrl = target.value.slice(0, 2e3);
    ...
    if (target.matches("[data-welcome-gallery-url]")) {
      event.layout.galleryUrls[Number(target.dataset.welcomeGalleryUrl)] = target.value.slice(0, 2e3);
      renderWelcomeMessagePreview(false);
    }
  ...
  function addLevelBoost() {
    ...
    const firstUnused = (state.directory.roles || []).find((role) => role.editable !== false && !state.config.leveling.roleBoosts.some((boost) => boost.roleId === role.id));
  ...
  // admin/client/workspace.js
  function renderGames() {
    if (!state.config?.counting) return;
    $("#lotteryChannel").innerHTML = channelOptions(
      state.config.games.lotteryChannelId,
      (channel) => channel.sendable === true && channel.kind !== "forum",
      "Use Games channel (automatic)"
    );
  ...
  function refreshDirty() {
    ...
    elements.saveState.textContent = saving ? templateMode ? "Saving template…" : reactionMode ? "Saving Reaction Role…" : "Applying changes…" : templateMode ? "Unsaved template changes" : reactionMode ? "Unsaved Reaction Role changes" : "Unsaved changes";
  ...
  elements.resetButton.addEventListener("click", () => {
    if (state.currentView === "message-templates") resetTemplateDraft();
      ...
      if (stored) {
        state.reactionRoleDraft = clone(stored);
        state.reactionRoleSavedSnapshot = reactionRoleSnapshot();
        renderReactionRoles();
        showToast("Unsaved Reaction Role changes reset.");
      }
  ...
  elements.gamesView.addEventListener("change", (event) => {
    ...
    if (channelIndex !== void 0) state.config.games.commandSettings[Number(channelIndex)].channelIds = [...event.target.selectedOptions].map((option) => option.value).filter(Boolean);
    if (commandIndex !== void 0) state.config.games.commandSettings[Number(commandIndex)].commands = [...event.target.selectedOptions].map((option) => option.value).filter(Boolean);
    refreshDirty();
  });
  ...
  for (const preview of [
    elements.levelingMessagePreview,
    elements.levelingAdditionalContainers,
    elements.welcomeMessagePreview,
    elements.welcomeAdditionalContainers,
    elements.templateMessagePreview,
    elements.templateAdditionalContainers,
    elements.reactionRoleMessagePreview,
    elements.reactionRoleAdditionalContainers,
    elements.xpDropMessagePreview,
    elements.xpDropClaimPreview
  ]) {
  ...
  elements.templateEditor.addEventListener("click", async (event) => {
    ...
    if (emoji) {
      ...
      openEmojiPicker({ type: spec.startsWith("button:") ? "template-button" : "template-option", spec, trigger: emoji }).catch((error) => showToast(error.message, "error"));
      return;
    }
    const configure = event.target.closest("[data-template-configure-action]");
    if (configure) {
      openTemplateActionDialog(configure.dataset.templateConfigureAction);
      return;
    }
    ...
    if (move) {
      ...
      if (target?.entries[target.index] && target.entries[to]) {
        const [item] = target.entries.splice(target.index, 1);
        target.entries.splice(to, 0, item);
        target.entries.forEach((entry, index) => {
          entry.sortOrder = index;
        });
      }
    ...
    if (removeDropdown) {
      ...
      state.templateDraft.controls.dropdowns.forEach((dropdown, index) => {
        dropdown.sortOrder = index;
      });
      state.templateSaveError = "";
      renderTemplateControls();
      renderTemplateControlPreview();
    ...
    if (moveDropdown) {
      ...
      const resolved = templateDropdownAt(id);
      const delta = Number(deltaText);
      const entries = state.templateDraft.controls.dropdowns;
      const to = resolved ? resolved.dropdownIndex + delta : -1;
      if (resolved && entries[to]) {
        const [dropdown] = entries.splice(resolved.dropdownIndex, 1);
        entries.splice(to, 0, dropdown);
  ...
  elements.templateAddControl.addEventListener("click", () => {
    const draft = state.templateDraft;
    if (!draft || draft.controls.type === "none") return;
    if (draft.controls.type === "button") {
    ...
    elements.templateResolvedPayload.textContent = JSON.stringify(resolvedTemplatePayloadPreview(), null, 2);
    refreshTemplateDirty();
    (draft.controls.type === "button" ? elements.templateControls.querySelector("[data-template-configure-action]:last-of-type") : elements.templateControls.querySelector(`[data-template-dropdown-card="${draft.controls.dropdowns.at(-1).id}"] [data-template-dropdown-placeholder]`))?.focus();
  });
  ...
  elements.templateVariablesToggle.addEventListener("click", () => {
    state.templateComposerPanel = state.templateComposerPanel === "variables" ? "" : "variables";
    renderTemplateComposerPanel();
  });
  elements.templateThumbnailAdd.addEventListener("click", () => {
    state.templateComposerPanel = state.templateComposerPanel === "thumbnail" ? "" : "thumbnail";
    renderTemplateComposerPanel();
  });
  elements.templateGalleryAdd.addEventListener("click", () => {
    state.templateComposerPanel = state.templateComposerPanel === "gallery" ? "" : "gallery";
    renderTemplateComposerPanel();
  });
  ...
  elements.reactionRoleEditor.querySelector(".reaction-role-tabs").addEventListener("click", (event) => {
    const tab = event.target.closest("[data-reaction-tab]");
    if (!tab) return;
    state.reactionRoleTab = tab.dataset.reactionTab;
    renderReactionRoleEditor();
  });
  elements.reactionRoleEditor.addEventListener("click", (event) => {
    const mode = event.target.closest("[data-reaction-mode]");
    if (mode && state.reactionRoleDraft) {
      state.reactionRoleDraft.interactionType = mode.dataset.reactionMode;
      renderReactionRoleEditor();
      return;
    }
    const emoji = event.target.closest("[data-reaction-emoji]");
    if (emoji) {
      const [type, index] = emoji.dataset.reactionEmoji.split(":");
      openEmojiPicker({ type: type === "button" ? "button" : "option", index: Number(index) });
      return;
    }
    const remove = event.target.closest("[data-rr-remove]");
    if (remove) {
      reactionRoleEntries().splice(Number(remove.dataset.rrRemove), 1);
      renderReactionRoleEditor();
      return;
    }
    const move = event.target.closest("[data-rr-move]");
    if (move) {
      const [from, delta] = move.dataset.rrMove.split(":").map(Number);
      const entries = reactionRoleEntries();
      const to = from + delta;
  ...
  elements.reactionRoleAddControl.addEventListener("click", () => {
    const draft = state.reactionRoleDraft;
    if (!draft || reactionRoleEntries().length >= 25) return;
  ...
  elements.reactionRoleContainerToggle.addEventListener("click", () => {
    if (!state.reactionRoleDraft) return;
    state.reactionRoleDraft.message.layout.container = !state.reactionRoleDraft.message.layout.container;
    renderReactionRoleEditor();
  });
  elements.reactionRoleAdditionalContainer.addEventListener("click", () => {
    const draft = state.reactionRoleDraft;
    if (!draft || draft.message.additionalContainers.length >= MAX_ADDITIONAL_MESSAGE_CONTAINERS) return;
    draft.message.additionalContainers.push(newAdditionalContainer(draft.message.layout.accentColor));
    renderReactionRoleEditor();
  });
  elements.reactionRoleVariablesToggle.addEventListener("click", () => {
    state.reactionRoleComposerPanel = state.reactionRoleComposerPanel === "variables" ? "" : "variables";
    renderReactionRoleComposerPanel();
  });
  elements.reactionRoleThumbnailToggle.addEventListener("click", () => {
    state.reactionRoleComposerPanel = state.reactionRoleComposerPanel === "thumbnail" ? "" : "thumbnail";
    renderReactionRoleComposerPanel();
  });
  elements.reactionRoleGalleryToggle.addEventListener("click", () => {
    state.reactionRoleComposerPanel = state.reactionRoleComposerPanel === "gallery" ? "" : "gallery";
    renderReactionRoleComposerPanel();
  });
  ...
  elements.reactionRoleComposerPanel.addEventListener("click", (event) => {
    const variable = event.target.closest("[data-insert-reaction-variable]");
    if (variable) return insertReactionRoleVariable(variable.dataset.insertReactionVariable);
    const draft = state.reactionRoleDraft;
    if (!draft) return;
    if (event.target.closest("[data-remove-reaction-thumbnail]")) {
      draft.message.layout.thumbnailEnabled = false;
      draft.message.layout.thumbnailUrl = "";
      renderReactionRoleEditor();
      return;
    }
    if (event.target.closest("[data-add-reaction-gallery]")) {
      if (draft.message.layout.galleryUrls.length < 10) draft.message.layout.galleryUrls.push("");
      renderReactionRoleEditor();
      return;
    }
    const remove = event.target.closest("[data-remove-reaction-gallery]");
    if (remove) {
      draft.message.layout.galleryUrls.splice(Number(remove.dataset.removeReactionGallery), 1);
      renderReactionRoleEditor();
    }
  });
  elements.reactionRoleComposerPanel.addEventListener("change", (event) => {
    const input = event.target.closest("[data-reaction-media-upload]");
    if (input) uploadReactionRoleMedia(input);
  });
  elements.reactionRoleAdditionalContainers.addEventListener("click", (event) => {
    const draft = state.reactionRoleDraft;
    if (!draft) return;
    const remove = event.target.closest("[data-remove-reaction-additional-container]");
    if (remove) {
      draft.message.additionalContainers.splice(Number(remove.dataset.removeReactionAdditionalContainer), 1);
      renderReactionRoleEditor();
      return;
    }
    const addGallery = event.target.closest("[data-add-reaction-additional-gallery]");
    if (addGallery) {
      const layout = draft.message.additionalContainers[Number(addGallery.dataset.addReactionAdditionalGallery)]?.layout;
      if (layout?.galleryUrls.length < 10) layout.galleryUrls.push("");
      renderReactionRoleEditor();
      return;
    }
    const removeGallery = event.target.closest("[data-remove-reaction-additional-gallery]");
    if (removeGallery) {
      const [containerIndex, mediaIndex] = removeGallery.dataset.removeReactionAdditionalGallery.split(":").map(Number);
      draft.message.additionalContainers[containerIndex]?.layout.galleryUrls.splice(mediaIndex, 1);
      renderReactionRoleEditor();
    }
  });
  elements.reactionRoleAdditionalContainers.addEventListener("change", (event) => {
    const input = event.target.closest("[data-reaction-media-upload]");
    if (input) uploadReactionRoleMedia(input);
  });
  ...
  document.querySelector(".emoji-picker-tabs").addEventListener("click", async (event) => {
    const tab = event.target.closest("[data-emoji-section]");
    if (!tab) return;
  ...
  elements.emojiPickerGrid.addEventListener("click", (event) => {
    const button = event.target.closest("[data-emoji-index]");
    if (button) applyPickedEmoji(state.emojiPickerItems[Number(button.dataset.emojiIndex)]);
  });
  ...
  elements.emojiPickerGrid.addEventListener("keydown", (event) => {
    ...
    const buttons = [...elements.emojiPickerGrid.querySelectorAll("[data-emoji-index]")];
    const current = buttons.indexOf(event.target.closest("[data-emoji-index]"));
    if (current < 0) return;
    event.preventDefault();
    const columns = Math.max(1, Math.round(elements.emojiPickerGrid.clientWidth / (buttons[0].offsetWidth + 6)));
  ...
  elements.emojiPickerDialog.addEventListener("click", (event) => {
    if (event.target !== elements.emojiPickerDialog) return;
    const box = elements.emojiPickerDialog.getBoundingClientRect();
    if (event.clientX < box.left || event.clientX > box.right || event.clientY < box.top || event.clientY > box.bottom) closeEmojiPicker();
  });
  ...
  function initializeInventory() {
      ...
      for (let index = 0; index < 50; index++) {
        ...
        if (!item) cell.setAttribute("aria-hidden", "true");
          ...
          cell.addEventListener("focus", () => {
            ...
            describe(item, box.left, box.bottom);
          });
          for (const event of ["blur", "pointerleave"]) cell.addEventListener(event, () => {
            tooltip.hidden = true;
          });
    ...
    async function loadItems() {
      ...
      get("inventoryStatus").textContent = "Loading inventory…";
      get("inventoryRefresh").disabled = true;
      ...
      try {
        ...
        items = data.items || [];
        page = Math.min(page, Math.max(1, Math.ceil(items.length / 50)));
        renderItems();
        ...
        if (request === inventoryRequest) get("inventoryStatus").textContent = `${error.message} Use Refresh to try again.`;
      } finally {
        if (request === inventoryRequest) {
          get("inventoryRefresh").disabled = false;
          get("inventoryGrid").setAttribute("aria-busy", "false");
        }
      }
    }
    function selectInventory(selected) {
      ...
      if (selected) url.searchParams.set("tab", "inventory");
    ...
    async function loadTickets(reset = false) {
      ...
      try {
        ...
        ticketPage = data.page;
        ticketMax = data.maxPages;
        get("ticketDate").replaceChildren(...[.../* @__PURE__ */ new Set([data.date, ...data.dates])].sort().reverse().map((date) => new Option(date, date, false, date === data.date)));
        get("ticketList").replaceChildren(...data.tickets.map((ticket) => {
          const card = document.createElement("article");
          card.className = `ticket-code prize-${ticket.rank || "none"}`;
        ...
        if (request === ticketRequest) get("ticketStatus").textContent = `${error.message} Change the filter or reopen tickets to retry.`;
      } finally {
        if (request === ticketRequest) {
          get("ticketPrevious").disabled = ticketPage <= 1;
          get("ticketNext").disabled = ticketPage >= ticketMax;
          get("ticketList").setAttribute("aria-busy", "false");
        }
      }
    }
    function openTickets() {
      tooltip.hidden = true;
      get("ticketDialog").showModal();
      loadTickets(true);
    }
    const actions = {
      profileCardTab: () => selectInventory(false),
      profileInventoryTab: () => selectInventory(true),
      inventoryRefresh: loadItems,
      ticketHistoryButton: openTickets,
      ticketClose: () => get("ticketDialog").close(),
      inventoryPrevious: () => {
        if (page > 1) {
          page--;
          renderItems();
      ...
      inventoryNext: () => {
        if (page < Math.ceil(items.length / 50)) {
          page++;
          renderItems();
        }
      },
    ...
    get("ticketSearch").addEventListener("input", () => {
      clearTimeout(searchTimer);
      ticketRequest++;
      searchTimer = setTimeout(() => loadTickets(true), 200);
    });
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape") tooltip.hidden = true;
    });
    new MutationObserver(() => {
      if (!get("profileShell").hidden && !get("webInventory").hidden) loadItems();
    }).observe(get("profileShell"), { attributes: true, attributeFilter: ["hidden"] });

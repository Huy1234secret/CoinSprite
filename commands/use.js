...
function usableOptions(userId) {
  ...
  return USABLE_ITEMS
    ...
    .slice(0, 25)
    .map((item) => ({ label: item.name.slice(0, 100), value: item.id, description: `Owned: ${formatNumber(getInventoryAmount(userId, item.id))}`.slice(0, 100), emoji: itemEmoji(item) }));
}
...
async function useItem(interaction, itemId, amount = 1, update = false) {
  if (itemId !== BUCKET_OF_WORMS_ID) {
    ...
    if (update && typeof interaction.update === 'function') await interaction.update(reply);
  ...
  const bucket = ITEM_BY_ID[BUCKET_OF_WORMS_ID];
  const worm = ITEM_BY_ID[WORM_ID];
  const bucketLabel = `${bucket?.emoji || ''} ${bucket?.name || 'Bucket of Worms'}`.trim();
  const wormLabel = `${worm?.emoji || ''} ${worm?.name || 'Worms'}`.trim();
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!rawItem) {
      await interaction.reply(chooserPayload(interaction));
      return;
    }
  ...
  async handleInteraction(interaction) {
    ...
    if (interaction.isStringSelectMenu?.() && interaction.customId?.startsWith('use:item:')) {
      if (ownerFromId(interaction.customId) !== interaction.user.id) {
        await interaction.reply({ content: 'You can only use your own item menu.', flags: EPHEMERAL_FLAG });
        return true;
      }

...
function drawEnemyProfileCircle(ctx, { x, y, radius, image, defenseImage, dead = false, hp, maxHp, power = 0, maxPower = 1, defense = 0 }) {
  ...
  if (!dead) {
    ...
    if (defense > 0) { const badgeR = Math.round(radius * .22); const bx = x + Math.round(radius * .60); const by = y + Math.round(radius * .60); ctx.beginPath(); ctx.arc(bx, by, badgeR, 0, Math.PI * 2); ctx.fillStyle = '#050505'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = Math.max(2, Math.round(radius * .025)); ctx.stroke(); if (defenseImage) ctx.drawImage(defenseImage, bx - badgeR + 3, by - badgeR + 3, (badgeR - 3) * 2, (badgeR - 3) * 2); ctx.fillStyle = '#fff'; ctx.font = `900 ${Math.round(badgeR * .70)}px Arial`; ctx.textAlign = 'center'; ctx.fillText(String(defense), bx + Math.round(badgeR * .35), by + Math.round(badgeR * .43)); }
...
async function createBattleImage(session) {
  ...
  const areaX = Math.round(width * .47);
  const areaY = Math.round(height * .03);
  const areaW = Math.round(width * .515);
  ...
  for (let index = 0; index < session.enemies.length; index += 1) {
    ...
    const maxTextWidth = Math.round(cellW * .88);
...
function targetRows(session) { const alive = session.enemies.filter((enemy) => enemy.hp > 0); const options = alive.map((enemy, index) => ({ label: `${enemy.name} ${index + 1}`, value: enemy.id, description: `HP ${enemy.hp}/${enemy.maxHp}`, emoji: SLIME_EMOJI_OBJECT })); return [actionRow(selectMenu(`journey:target:${session.userId}:${session.id}`, 'Select enemy', options.length ? options : [{ label: 'No enemies left', value: 'none' }], options.length === 0)), actionRow(button(`journey:back:${session.userId}:${session.id}`, 'Back', 2))]; }

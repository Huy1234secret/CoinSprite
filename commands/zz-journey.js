...
const JUNGLE_SLIME_EMOJI = { id: '1501095601432170506', name: 'ENJungleSlime' };
const JUNGLE_SLIME = '<:ENJungleSlime:1501095601432170506>';
const JUNGLE_SLIME_URL = 'https://cdn.discordapp.com/emojis/1501095601432170506.png?size=256&quality=lossless';
const DEFENSE_EMOJI_TEXT = '<:SBDefense:1501156914665488486>';
const DEFENSE_EMOJI_URL = 'https://cdn.discordapp.com/emojis/1501156914665488486.png?size=128&quality=lossless';
...
const STAGES = [
  {
    ...
    enemies: [
      {
        ...
        name: 'Jungle Slime',
        emoji: JUNGLE_SLIME,
        emojiObject: JUNGLE_SLIME_EMOJI,
        imageUrl: JUNGLE_SLIME_URL,
        rarity: 'Common',
        count: 2,
        hp: 10,
        power: 0,
    ...
    drops: [
      ...
      { itemId: 'jungle_goo', name: 'Jungle Goo', emoji: '<:ITJungleGoo:1501156916737609798>', amount: 2, chance: 0.10, chained: true },
    ],
  },
  { id: 'mossy_ruins', name: 'Mossy Ruins', chapter: 1, stage: 2, stars: 0, completed: false, reward: { coins: 6, exp: 6 }, enemies: [{ id: 'jungle_slime', name: 'Jungle Slime', emoji: JUNGLE_SLIME, emojiObject: JUNGLE_SLIME_EMOJI, imageUrl: JUNGLE_SLIME_URL, rarity: 'Common', count: 2, hp: 10, power: 0, maxPower: 10, defense: 0, defenseTurns: 0 }], drops: [] },
  { id: 'ancient_canopy', name: 'Ancient Canopy', chapter: 1, stage: 3, stars: 0, completed: false, reward: { coins: 8, exp: 8 }, enemies: [{ id: 'jungle_slime', name: 'Jungle Slime', emoji: JUNGLE_SLIME, emojiObject: JUNGLE_SLIME_EMOJI, imageUrl: JUNGLE_SLIME_URL, rarity: 'Common', count: 2, hp: 10, power: 0, maxPower: 10, defense: 0, defenseTurns: 0 }], drops: [] },
];
...
function aggregateDrops(drops) { const map = new Map(); for (const drop of drops || []) { const key = drop.itemId; const existing = map.get(key) || { ...drop, amount: 0 }; existing.amount += Math.max(0, Math.floor(Number(drop.amount) || 0)); map.set(key, existing); } return [...map.values()]; }
...
function drawMapInfo(ctx, stage, width, height) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    ctx.fillStyle = i < Math.max(0, Math.min(3, stage.stars || 0)) ? '#f5c542' : '#000';
    ctx.fill();
...
function drawSimpleProfileCircle(ctx, x, y, radius, image, fallback = '?', border = 'rgba(255,255,255,.9)') {
  ...
  ctx.shadowBlur = Math.round(radius * .20);
  ctx.shadowOffsetY = Math.round(radius * .08);
  ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fillStyle = '#050505'; ctx.fill(); ctx.restore();
...
async function drawEnemyList(ctx, stage, width, height) {
  ...
  const titleY = Math.round(height * .30);
  const circleY = Math.round(height * .58);
  const radius = Math.round(86 * scale);
  const gap = Math.round(radius * 2.02);
  ...
  for (let index = 0; index < enemies.length; index += 1) {
    ...
    const enemyImage = await loadRemoteImage(enemy.imageUrl || JUNGLE_SLIME_URL);
    const x = startX + (index * gap);
    drawSimpleProfileCircle(ctx, x, circleY, radius, enemyImage);
    setTextShadow(ctx, 10); ctx.font = `800 ${Math.round(radius * .38)}px Arial`; strokedText(ctx, enemy.name, x, circleY + Math.round(radius * 1.47), '#fff', 'rgba(0,0,0,.9)', Math.round(radius * .10)); clearTextShadow(ctx);
    if (enemy.count > 1) {
      const badgeX = x + Math.round(56 * scale); const badgeY = circleY + Math.round(54 * scale);
      ctx.beginPath(); ctx.arc(badgeX, badgeY, Math.round(36 * scale), 0, Math.PI * 2); ctx.fillStyle = '#000'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = Math.max(3, Math.round(5 * scale)); ctx.stroke(); ctx.fillStyle = '#fff'; ctx.font = `900 ${Math.round(27 * scale)}px Arial`; ctx.fillText(`x${enemy.count}`, badgeX, badgeY + Math.round(9 * scale));
    }
  }
  ctx.textAlign = 'left';
}
function drawVerticalRing(ctx, x, y, radius, value, max, color, width) {
  ...
  for (let i = 0; i < segments; i += 1) {
    const a1 = -Math.PI / 2 + (Math.PI * 2 * i / segments); const a2 = -Math.PI / 2 + (Math.PI * 2 * (i + 1.05) / segments); const mid = (a1 + a2) / 2; const midY = y + Math.sin(mid) * lineRadius; const cutoffY = (y - lineRadius) + (lineRadius * 2 * (1 - percent)); ctx.beginPath(); ctx.arc(x, y, lineRadius, a1, a2); ctx.strokeStyle = midY >= cutoffY ? color : '#050505'; ctx.stroke();
  }
  ctx.restore();
}
function drawEnemyProfileCircle(ctx, { x, y, radius, image, defenseImage, dead = false, hp, maxHp, power = 0, maxPower = 1, defense = 0 }) {
  const hpWidth = Math.max(9, Math.round(radius * .075));
  const powWidth = Math.max(6, Math.round(radius * .052));
  const whiteWidth = Math.max(3, Math.round(radius * .026));
  ...
  if (!dead) {
    ...
    ctx.beginPath(); ctx.arc(x, y, whiteRadius, 0, Math.PI * 2); ctx.lineWidth = whiteWidth; ctx.strokeStyle = 'rgba(255,255,255,.95)'; ctx.stroke();
    if (defense > 0) {
      const badgeR = Math.round(radius * .24); const bx = x + Math.round(radius * .60); const by = y + Math.round(radius * .60);
      ctx.beginPath(); ctx.arc(bx, by, badgeR, 0, Math.PI * 2); ctx.fillStyle = '#050505'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = Math.max(2, Math.round(radius * .025)); ctx.stroke();
      if (defenseImage) ctx.drawImage(defenseImage, bx - badgeR + 3, by - badgeR + 3, (badgeR - 3) * 2, (badgeR - 3) * 2);
      ctx.fillStyle = '#fff'; ctx.font = `900 ${Math.round(badgeR * .72)}px Arial`; ctx.textAlign = 'center'; ctx.fillText(String(defense), bx + Math.round(badgeR * .35), by + Math.round(badgeR * .45));
    }
...
function drawStatLine(ctx, x, y, width, height, value, max, color, label) {
  const percent = Math.max(0, Math.min(1, value / Math.max(1, max)));
  roundedRectPath(ctx, x, y, width, height, height / 2); ctx.fillStyle = '#050505'; ctx.fill();
...
function enemyGrid(count) { if (count <= 1) return { cols: 1, rows: 1 }; if (count <= 2) return { cols: 2, rows: 1 }; if (count <= 4) return { cols: 2, rows: 2 }; if (count <= 6) return { cols: 3, rows: 2 }; return { cols: 3, rows: Math.ceil(count / 3) }; }
...
function drawNameAndLevel(ctx, name, level, x, y, scale, align = 'left') { ctx.textAlign = align; setTextShadow(ctx, 13); ctx.font = `900 ${Math.round(52 * scale)}px Arial`; strokedText(ctx, name, x, y, '#fff', 'rgba(0,0,0,.82)', Math.round(10 * scale)); ctx.font = `900 ${Math.round(30 * scale)}px Arial`; strokedText(ctx, `Level ${level}`, x, y + Math.round(42 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(6 * scale)); clearTextShadow(ctx); }
async function createBattleImage(session) {
  const canvas = await loadBannerCanvas(); const ctx = canvas.getContext('2d'); const width = canvas.width; const height = canvas.height; const scale = Math.max(.7, Math.min(width / 2048, height / 330)); const avatar = await loadRemoteImage(session.avatarUrl); const defenseImage = await loadRemoteImage(DEFENSE_EMOJI_URL);
  const playerRadius = Math.round(126 * scale); const playerX = Math.round(width * .085); const playerY = Math.round(height * .54); const playerTextX = Math.round(width * .185); const playerTextY = Math.round(height * .23);
  drawSimpleProfileCircle(ctx, playerX, playerY, playerRadius, avatar, '?', 'rgba(255,255,255,.95)'); drawNameAndLevel(ctx, session.username, session.player.level, playerTextX, playerTextY, scale);
  ...
  const grid = enemyGrid(Math.max(1, session.enemies.length)); const areaX = Math.round(width * .47); const areaY = Math.round(height * .03); const areaW = Math.round(width * .515); const areaH = Math.round(height * .93); const cellW = areaW / grid.cols; const cellH = areaH / grid.rows;
  for (let index = 0; index < session.enemies.length; index += 1) {
    const enemy = session.enemies[index]; const col = index % grid.cols; const row = Math.floor(index / grid.cols); const centerX = Math.round(areaX + (col * cellW) + (cellW * .36)); const centerY = Math.round(areaY + (row * cellH) + (cellH * .52)); const radius = Math.min(Math.round(146 * scale), Math.round(Math.min(cellW, cellH) * .49)); const enemyImage = await loadRemoteImage(enemy.imageUrl || JUNGLE_SLIME_URL);
    drawEnemyProfileCircle(ctx, { x: centerX, y: centerY, radius, image: enemyImage, defenseImage, dead: enemy.hp <= 0, hp: enemy.hp, maxHp: enemy.maxHp, power: enemy.power, maxPower: enemy.maxPower, defense: enemy.defense });
    const infoX = centerX + radius + Math.round(18 * scale); const infoY = centerY - Math.round(20 * scale); drawNameAndLevel(ctx, enemy.name, enemy.level || 1, infoX, infoY, Math.max(.62, scale * .72));
    if (enemy.status) { setTextShadow(ctx, 7); ctx.font = `900 ${Math.round(18 * scale)}px Arial`; strokedText(ctx, enemy.status, infoX, infoY + Math.round(70 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(4 * scale)); clearTextShadow(ctx); }
  }
  return await canvas.encode('png');
}

function homePayload(interaction, stage, imageAttachment) { const completed = STAGES.filter((item) => item.completed).length; return { flags: COMPONENTS_V2_FLAG, files: imageAttachment ? [imageAttachment] : [], components: [{ type: 17, accent_color: WHITE_ACCENT, components: [text(`## ${interaction.user.username}'s Journey`), mediaGallery(`attachment://${STAGE_IMAGE_NAME}`), text(`-# You have completed ${completed} / ${STAGES.length} stages so far\n* ${formatStageReward(stage)}`), separator(), actionRow(button(`journey:play:${interaction.user.id}:${stage.id}`, 'Play', 3), button(`journey:change:${interaction.user.id}`, 'Change Stages', 2, true)), actionRow(selectMenu(`journey:chapter:${interaction.user.id}`, 'More Chapter soon', [{ label: 'More Chapter soon', value: 'soon', description: 'New chapters will be added later.' }], true))] }] }; }
...
function targetRows(session) { const alive = session.enemies.filter((enemy) => enemy.hp > 0); const options = alive.map((enemy, index) => ({ label: `${enemy.name} ${index + 1}`, value: enemy.id, description: `HP ${enemy.hp}/${enemy.maxHp}`, emoji: JUNGLE_SLIME_EMOJI })); return [actionRow(selectMenu(`journey:target:${session.userId}:${session.id}`, 'Select enemy', options.length ? options : [{ label: 'No enemies left', value: 'none' }], options.length === 0)), actionRow(button(`journey:back:${session.userId}:${session.id}`, 'Back', 2))]; }
...
async function finishPayload(interaction, session, win) { const image = await createBattleImage(session); let rewardText = '-# You have been defeated...'; if (win) { session.pendingDrops = rollDrops(session.stage); const reward = await awardWinRewards(interaction, session); const dropText = reward.drops.length ? `\n-# Enemy drops:\n${reward.drops.map(formatDropLine).join('\n')}` : ''; rewardText = `${session.actionLog.join('\n')}\n-# You have defeated all enemies and got ${formatNumber(reward.coins)} ${COIN}, ${formatNumber(reward.exp)} exp.${dropText}`; } else { rewardText = `${session.actionLog.join('\n')}\n-# You have been defeated and lost all drops...`; }
  return { flags: COMPONENTS_V2_FLAG, files: [new AttachmentBuilder(image, { name: BATTLE_IMAGE_NAME })], components: [{ type: 17, accent_color: win ? GREEN_ACCENT : RED_ACCENT, components: [text(win ? `## ${session.username} has defeated stage ${stageSubtitle(session.stage)}!` : `## ${session.username} has failed stage ${stageSubtitle(session.stage)}!`), mediaGallery(`attachment://${BATTLE_IMAGE_NAME}`), text(rewardText), separator(), ...(win ? [actionRow(button(`journey:home:${session.userId}`, 'Home', 2))] : [actionRow(button(`journey:retry:${session.userId}:${session.stage.id}`, 'Retry', 2), button(`journey:home:${session.userId}`, 'Home', 2))])] }] };
}
...
function useJungleSlimeMove(enemy, session) { if (enemy.power >= 10) { enemy.defense = Math.min(10, Math.max(enemy.defense || 0, 1)); enemy.defenseTurns = 2; enemy.power = Math.max(0, enemy.power - 10); enemy.status = `${DEFENSE_EMOJI_TEXT} ${enemy.defense}`; return `-# Jungle Slime absorbs jungle leaves, gaining a protective shield last 2 turns.`; } const raw = rand(2, 3); const damage = raw; session.player.hp = Math.max(0, session.player.hp - damage); const gain = rand(1, 3); enemy.power = Math.min(enemy.maxPower, enemy.power + gain); return `-# The Jungle Slime jumped toward ${session.username} and dealt ${damage} damage.`; }
...
module.exports = {
  ...
  async handleInteraction(interaction) {
    ...
    if (action === 'attack') { const value = interaction.values?.[0]; if (value === 'punch') { session.pendingAttack = 'punch'; await interaction.update(await targetPayload(session)); return true; } await interaction.reply({ content: 'That action is not ready yet.', flags: EPHEMERAL_FLAG }); return true; }
    if (action === 'back') { await interaction.update(await battlePayload(session)); return true; }
    if (action === 'target') { const targetId = interaction.values?.[0]; return applyPunch(interaction, session, targetId); }
    return false;
  },
};

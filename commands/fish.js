...
const LOADING_FISH = '<a:SBLoadingfish:1499381238656667728>';
...
function scheduleFishMove(sessionId) { const session = activeFishingSessions.get(sessionId); if (!session || session.done) return; const delay = session.fishPosition === session.greenPosition ? 4_000 : 2_000; session.timer = setTimeout(async () => { const current = activeFishingSessions.get(sessionId); if (!current || current.done) return; let nextPosition = randomInt(0, 4); if (nextPosition === current.fishPosition) nextPosition = (nextPosition + randomInt(1, 4)) % 5; if (nextPosition === current.greenPosition) { current.redZoneStreak = 0; current.targetRedZoneStreak = randomInt(3, 5); } else { current.redZoneStreak = Math.max(0, Math.floor(Number(current.redZoneStreak) || 0)) + 1; if (current.redZoneStreak >= Math.max(3, Math.floor(Number(current.targetRedZoneStreak) || 3))) { nextPosition = current.greenPosition; current.redZoneStreak = 0; current.targetRedZoneStreak = randomInt(3, 5); } } current.fishPosition = nextPosition; await current.message?.edit(buildFishingPayload(current)).catch(() => null); scheduleFishMove(sessionId); }, delay); }
async function startFishing(interaction) { const req = hasFishingRequirements(interaction.user.id); if (!req.ready) { await interaction.reply({ content: `You need at least ${req.missing.join(' and ')} to start fishing.`, flags: EPHEMERAL_FLAG }); return; } if (!removeInventoryItem(interaction.user.id, WORM_ID, 1)) { await interaction.reply({ content: 'You need at least 1 Worm to start fishing.', flags: EPHEMERAL_FLAG }); return; } const rod = equipNextFishingRod(interaction.user.id); if (!rod) { await interaction.reply({ content: 'You need at least 1 Fishing rod to start fishing.', flags: EPHEMERAL_FLAG }); return; } const upgrades = getFishingUpgrades(interaction.user.id); const fish = rollFish(upgrades.luck); const session = { id: makeSessionId(), userId: interaction.user.id, fish, progress: 20, greenPosition: randomInt(0, 4), fishPosition: randomInt(0, 4), rodDurability: rod.durability, strength: getTotalRodStrength(upgrades.strength), finalValue: getFishFinalValue(fish, upgrades.value), personalChance: getSessionFishChancePercent(fish, upgrades.luck), redZoneStreak: 0, targetRedZoneStreak: randomInt(3, 5), message: interaction.message, done: false, timer: null }; activeFishingSessions.set(session.id, session); await interaction.update(buildCastingPayload()); session.message = interaction.message; setTimeout(async () => { const current = activeFishingSessions.get(session.id); if (!current || current.done) return; await current.message?.edit(buildFishingPayload(current)).catch(() => null); scheduleFishMove(session.id); }, 3_000); }
async function reel(interaction) { const sessionId = interaction.customId.split(':')[3]; const session = activeFishingSessions.get(sessionId); if (!session || session.done) { await interaction.reply({ content: 'This fishing session is no longer active.', flags: EPHEMERAL_FLAG }); return; } if (session.userId !== interaction.user.id) { await interaction.reply({ content: 'You can only reel your own fish.', flags: EPHEMERAL_FLAG }); return; } const applyRodDamage = async (multiplier, reason) => { const damage = Math.max(1, Math.ceil(session.fish.durabilityDamage * multiplier)); const rodResult = damageEquippedRod(interaction.user.id, damage); session.rodDurability = rodResult.rod?.durability ?? Math.max(0, session.rodDurability - damage); if (!rodResult.broke) return false; session.done = true; cleanupSession(session.id); fishCooldowns.set(interaction.user.id, Date.now() + FISH_COOLDOWN_MS);
  await interaction.update(buildResultPayload(interaction, session, false, reason || `Your fishing rod broke and ${session.fish.emoji} ${session.fish.name} has escaped`)); await announceRareCatch(interaction, session, false); return true; }; if (session.fishPosition === session.greenPosition) { session.progress = clamp(session.progress + calculateFishingProgressGain(session.strength, session.fish.rodStrengthRequirement), 0, 100); if (await applyRodDamage(0.10)) return; } else { session.progress = clamp(session.progress - randomInt(20, 30), 0, 100); if (await applyRodDamage(0.50)) return; } if (session.progress >= 100) { session.done = true; cleanupSession(session.id); if (await applyRodDamage(0.25, `Your fishing rod broke while landing ${session.fish.emoji} ${session.fish.name}`)) return; addInventoryItem(interaction.user.id, session.fish.id, 1); const fishStats = recordFishCaught(interaction.user.id, session.fish.id, 1); await unlockFishAchievements(interaction.channel, interaction.user, fishStats, session.fish, FISHES.length); fishCooldowns.set(interaction.user.id, Date.now() + FISH_COOLDOWN_MS);
    await interaction.update(buildResultPayload(interaction, session, true));
    await announceRareCatch(interaction, session, true);
    return; } if (session.progress <= 0) { session.done = true; cleanupSession(session.id); if (await applyRodDamage(1)) return; fishCooldowns.set(interaction.user.id, Date.now() + FISH_COOLDOWN_MS);
    await interaction.update(buildResultPayload(interaction, session, false, `You have failed to reel ${session.fish.emoji} ${session.fish.name} and it escaped`));

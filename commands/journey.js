...
function drawVerticalRing(ctx, x, y, radius, value, max, color, width) {
  ...
  if (percent < 1) {
    ...
    ctx.fillRect(x - radius - width, y - radius - width, (radius + width) * 2, (radius + width) * 2 * (1 - percent));
    ctx.globalCompositeOperation = 'source-over';
  }
  ctx.restore();
}

function drawProfileCircle(ctx, { x, y, radius, emoji, image, hp, maxHp, power = 0, maxPower = 1, fontSize, healthWidth, powerWidth, whiteWidth, drawPowerRing = false }) {
  const hpWidth = healthWidth || Math.max(8, Math.round(radius * .13));
  const powWidth = powerWidth || Math.max(4, Math.round(radius * .055));
  const innerWhiteWidth = whiteWidth || Math.max(2, Math.round(radius * .025));
  const hpRadius = radius - (hpWidth / 2);
  const powerRadius = radius - hpWidth - (powWidth / 2) - Math.max(2, Math.round(radius * .03));
  const whiteRadius = powerRadius - (powWidth / 2) - (innerWhiteWidth / 2) - Math.max(1, Math.round(radius * .015));
  ...
  ctx.beginPath();
  ctx.arc(x, y, radius - hpWidth - powWidth - innerWhiteWidth - 6, 0, Math.PI * 2);
  ctx.fillStyle = 'rgba(18,18,18,.92)';
  ctx.fill();
  ctx.restore();

  const contentRadius = radius - hpWidth - powWidth - innerWhiteWidth - 8;
  if (image) {
    ctx.save();
    ctx.beginPath();
    ...
    ctx.font = `900 ${fontSize || Math.round(radius * .72)}px Arial`;
    ctx.fillText(emoji || '?', x, y + Math.round((fontSize || radius * .72) * .34));
    clearTextShadow(ctx);
  }
...
function drawEnemyList(ctx, stage, width, height) {
  ...
  strokedText(ctx, 'Enemies', right, titleY, '#fff', 'rgba(0,0,0,.82)', Math.round(10 * scale));
  clearTextShadow(ctx);
  ctx.textAlign = 'center';
  enemies.forEach((enemy, index) => {
    const x = startX + (index * gap);
    drawProfileCircle(ctx, { x, y: circleY, radius, emoji: '?', hp: 1, maxHp: 1, power: 0, maxPower: 1, fontSize: Math.round(54 * scale), drawPowerRing: false });
...
function drawStatLine(ctx, x, y, width, height, value, max, color, label) {
  ...
  ctx.lineWidth = Math.max(2, Math.round(height * .12));
  ctx.stroke();
  if (label) {
    setTextShadow(ctx, 4);
    ctx.fillStyle = '#fff';
    ctx.textAlign = 'center';
    ctx.font = `900 ${Math.round(height * .72)}px Arial`;
    ctx.fillText(label, x + width / 2, y + Math.round(height * .77));
    clearTextShadow(ctx);
  }
}
...
async function createBattleImage(session) {
  ...
  strokedText(ctx, `Level ${session.player.level}`, left + Math.round(245 * scale), top + Math.round(73 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(6 * scale));
  clearTextShadow(ctx);

  const playerRadius = Math.round(98 * scale);
  const playerX = left + Math.round(105 * scale);
  const playerY = top + Math.round(125 * scale);
  drawProfileCircle(ctx, { x: playerX, y: playerY, radius: playerRadius, image: avatar, emoji: '?', hp: session.player.hp, maxHp: session.player.maxHp, power: 0, maxPower: session.player.maxPower, drawPowerRing: false, healthWidth: Math.round(12 * scale), powerWidth: Math.round(6 * scale), whiteWidth: Math.round(4 * scale) });

  const statX = left + Math.round(245 * scale);
  drawStatLine(ctx, statX, top + Math.round(98 * scale), Math.round(455 * scale), Math.round(27 * scale), session.player.hp, session.player.maxHp, '#ff4040', `${session.player.hp}/${session.player.maxHp} HP`);
  drawStatLine(ctx, statX, top + Math.round(138 * scale), Math.round(455 * scale), Math.round(27 * scale), session.player.power, session.player.maxPower, '#00ffff', `${session.player.power}/${session.player.maxPower} Power`);
  if (session.player.status) {
    setTextShadow(ctx, 8);
    ctx.font = `900 ${Math.round(25 * scale)}px Arial`;
    strokedText(ctx, session.player.status, left, top + Math.round(225 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(5 * scale));
    clearTextShadow(ctx);
  }
  ...
  const areaY = Math.round(height * .07);
  const areaW = Math.round(width * .47);
  const areaH = Math.round(height * .86);
  ...
  alive.forEach((enemy, index) => {
    ...
    const radius = Math.min(Math.round(98 * scale), Math.round(Math.min(cellW, cellH) * .38));
    drawProfileCircle(ctx, { x: centerX, y: centerY, radius, emoji: enemy.emoji || '?', hp: enemy.hp, maxHp: enemy.maxHp, power: enemy.power, maxPower: enemy.maxPower, fontSize: Math.round(radius * .72), healthWidth: Math.round(13 * scale), powerWidth: Math.round(7 * scale), whiteWidth: Math.round(4 * scale), drawPowerRing: true });
    if (enemy.status) {
      setTextShadow(ctx, 7);
      ctx.font = `900 ${Math.round(18 * scale)}px Arial`;
      strokedText(ctx, enemy.status, centerX, centerY + radius + Math.round(35 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(4 * scale));
      clearTextShadow(ctx);
    }
  });
...
async function startEnemyTurn(interaction, session) { session.turn = 'enemy'; session.actionLog.push(`-# Enemy turn <t:${Math.floor((Date.now() + 3000) / 1000)}:R>`); await editBattleMessage(interaction.message, session, WHITE_ACCENT, true); const alive = session.enemies.filter((enemy) => enemy.hp > 0); alive.forEach((enemy, index) => { queueTimer(session, async () => { if (!activeBattles.has(session.id) || session.turn !== 'enemy') return; let damage = 1; let attackName = 'Punch'; enemy.power = Math.min(enemy.maxPower, enemy.power + 1); if (enemy.power >= enemy.maxPower) { attackName = 'Power Punch'; damage = 3; enemy.power = 0; } session.player.hp = Math.max(0, session.player.hp - damage); session.actionLog.push(`-# ${enemy.name} ${enemy.emoji} used **${attackName}** and dealt ${damage} damage.`); if (session.player.hp <= 0) { cleanupSession(session); await interaction.message.edit(await finishPayload(session, false)).catch(() => null); return; } await editBattleMessage(interaction.message, session, WHITE_ACCENT, true); }, 3000 + (index * 2000)); }); queueTimer(session, async () => { if (!activeBattles.has(session.id) || session.turn !== 'enemy') return; session.turn = 'player'; session.playerActions = 0; session.actionLog = [`-# It's your turn`]; await editBattleMessage(interaction.message, session); }, 3000 + (alive.length * 2000) + 800); }
async function handlePunch(interaction, session) { if (session.turn !== 'player') { await interaction.reply({ content: 'It is not your turn.', flags: EPHEMERAL_FLAG }); return true; } await interaction.deferUpdate(); const target = session.enemies.find((enemy) => enemy.hp > 0); if (!target) return true; const damage = rand(2, 4); target.hp = Math.max(0, target.hp - damage); session.player.power = Math.min(session.player.maxPower, session.player.power + 2); session.playerActions += 1; session.actionLog = [`-# ${session.username} used **Punch** onto **${target.name} ${target.emoji}** and deal ${damage} damage.`]; if (!session.enemies.some((enemy) => enemy.hp > 0)) { cleanupSession(session); await interaction.message.edit(await finishPayload(session, true)).catch(() => null); return true; } if (session.playerActions >= 2) { await startEnemyTurn(interaction, session); return true; } await editBattleMessage(interaction.message, session); return true; }

...
function drawSimpleProfileCircle(ctx, x, y, radius, image, fallback = '?', border = 'rgba(255,255,255,.9)') {
  ...
  if (image) {
    ctx.save();
    ctx.beginPath();
    ctx.arc(x, y, radius - Math.max(5, Math.round(radius * .06)), 0, Math.PI * 2);
    ctx.clip();
    ctx.drawImage(image, x - radius, y - radius, radius * 2, radius * 2);
    ctx.restore();
  } else {
    ctx.fillStyle = '#fff';
    ctx.font = `900 ${Math.round(radius * .70)}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText(fallback, x, y + Math.round(radius * .25));
  }
  ctx.beginPath();
  ctx.arc(x, y, radius, 0, Math.PI * 2);
  ctx.strokeStyle = border;
  ctx.lineWidth = Math.max(5, Math.round(radius * .08));
  ctx.stroke();
}
...
async function drawEnemyList(ctx, stage, width, height) {
  ...
  enemies.forEach((enemy, index) => {
    ...
    ctx.font = `800 ${Math.round(radius * .42)}px Arial`;
    strokedText(ctx, enemy.name, x, circleY + Math.round(radius * 1.52), '#fff', 'rgba(0,0,0,.9)', Math.round(radius * .10));
    clearTextShadow(ctx);
    if (enemy.count > 1) {
...
function drawEnemyProfileCircle(ctx, { x, y, radius, image, dead = false, hp, maxHp, power = 0, maxPower = 1, healthWidth, powerWidth, whiteWidth }) {
  const hpWidth = healthWidth || Math.max(7, Math.round(radius * .075));
  const powWidth = powerWidth || Math.max(4, Math.round(radius * .045));
  const innerWhiteWidth = whiteWidth || Math.max(2, Math.round(radius * .022));
  ...
  if (!dead && image) {
    ...
    ctx.fillStyle = '#fff';
    ctx.font = `900 ${Math.round(radius * .65)}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText('?', x, y + Math.round(radius * .22));
  }
  ...
  if (!dead) {
    ...
    ctx.beginPath();
    ctx.arc(x, y, radius - (hpWidth / 2), 0, Math.PI * 2);
...
function drawNameAndLevel(ctx, name, level, x, y, scale, align = 'left') {
  ctx.textAlign = align;
  setTextShadow(ctx, 13);
  ctx.font = `900 ${Math.round(52 * scale)}px Arial`;
  strokedText(ctx, name, x, y, '#fff', 'rgba(0,0,0,.82)', Math.round(10 * scale));
  ctx.font = `900 ${Math.round(30 * scale)}px Arial`;
  strokedText(ctx, `Level ${level}`, x, y + Math.round(42 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(6 * scale));
  clearTextShadow(ctx);
}
...
async function createBattleImage(session) {
  ...
  const playerRadius = Math.round(126 * scale);
  const playerX = Math.round(width * .085);
  ...
  drawNameAndLevel(ctx, session.username, session.player.level, playerTextX, playerTextY, scale);
  const statX = playerTextX;
  const barW = Math.round(505 * scale);
  const barH = Math.round(34 * scale);
  drawStatLine(ctx, statX, playerTextY + Math.round(67 * scale), barW, barH, session.player.hp, session.player.maxHp, '#ff4040', `${session.player.hp}/${session.player.maxHp} HP`);
  drawStatLine(ctx, statX, playerTextY + Math.round(114 * scale), barW, barH, session.player.power, session.player.maxPower, '#00ffff', `${session.player.power}/${session.player.maxPower} Power`);
  if (session.player.status) {
    setTextShadow(ctx, 8);
    ctx.font = `900 ${Math.round(25 * scale)}px Arial`;
    strokedText(ctx, session.player.status, playerTextX, playerTextY + Math.round(178 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(5 * scale));
    clearTextShadow(ctx);
  }

  const grid = enemyGrid(Math.max(1, session.enemies.length));
  const areaX = Math.round(width * .48);
  const areaY = Math.round(height * .03);
  const areaW = Math.round(width * .50);
  const areaH = Math.round(height * .93);
  const cellW = areaW / grid.cols;
  const cellH = areaH / grid.rows;

  session.enemies.forEach((enemy, index) => {
    const col = index % grid.cols;
    const row = Math.floor(index / grid.cols);
    const centerX = Math.round(areaX + (col * cellW) + (cellW * .34));
    const centerY = Math.round(areaY + (row * cellH) + (cellH * .52));
    const radius = Math.min(Math.round(132 * scale), Math.round(Math.min(cellW, cellH) * .43));
    drawEnemyProfileCircle(ctx, { x: centerX, y: centerY, radius, image: enemyImage, dead: enemy.hp <= 0, hp: enemy.hp, maxHp: enemy.maxHp, power: enemy.power, maxPower: enemy.maxPower, healthWidth: Math.round(10 * scale), powerWidth: Math.round(7 * scale), whiteWidth: Math.round(4 * scale) });
    const infoX = centerX + radius + Math.round(24 * scale);
    const infoY = centerY - Math.round(20 * scale);
    drawNameAndLevel(ctx, enemy.name, enemy.level || 1, infoX, infoY, Math.max(.62, scale * .72));
    if (enemy.status) {
      setTextShadow(ctx, 7);
      ctx.font = `900 ${Math.round(18 * scale)}px Arial`;
      strokedText(ctx, enemy.status, infoX, infoY + Math.round(70 * scale), '#fff', 'rgba(0,0,0,.82)', Math.round(4 * scale));
      clearTextShadow(ctx);
    }
  });

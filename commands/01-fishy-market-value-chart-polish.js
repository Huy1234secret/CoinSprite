...
function patchFishyMarketSource(source) {
  ...
  return \`-# **You've sold \\u00d7\${amount} \${item.name} \${item.emoji} - \${rarityLabel(item.rarity)} - \${value} \${FISH_COIN}**\`;
    ...
    .replace(/function renderChartImage\(entry, itemName\) \{[\s\S]*?\n\}\n\nfunction renderValueChart/, `function niceChartStep(value) {
...
function formatChartValue(value) {
  ...
  return num.toFixed(abs < 10 ? 1 : 0).replace(/\\.0$/, '');
}

function formatChartAge(at, now) {
  const diffMs = Math.max(0, now - (Number(at) || now));
...
function chartScale(values, baseValue) {
  const numericValues = values.map((value) => Number(value) || 0);
  const allValues = numericValues.concat([Number(baseValue) || 0]);
  ...
  const spread = Math.max(0, dataHigh - dataLow);
  const padding = spread > 0 ? Math.max(spread * 0.22, integerOnly ? 1 : 0.5) : Math.max(2, Math.ceil(Math.max(1, dataHigh) * 0.25));
  let min = Math.max(0, dataLow - padding);
  let max = dataHigh + padding;
  let step = niceChartStep((max - min) / 4);
...
function drawChartPill(ctx, x, y, text, fill, stroke, textColor) {
...
function renderChartImage(entry, itemName) {
  ...
  const scale = chartScale(values, baseValue);
  const x0 = 92;
  const y0 = 354;
  const w = 756;
  const h = 206;
  ...
  ctx.textAlign = 'right';
  ctx.fillText(formatChartValue(currentValue), 786, 76);
  ctx.fillStyle = '#91a2be';
  ctx.font = '700 13px Arial';
  ...
  for (const tick of scale.ticks) {
    ...
    ctx.fillText(formatChartValue(tick), x0 - 12, y + 4);
  }
  ctx.setLineDash([]);
  ...
  const labelX = Math.min(lastPoint.x + 14, x0 + w - 86);
  ctx.fillText(formatChartValue(lastPoint.value) + ' coins', labelX, Math.max(y0 - h + 22, lastPoint.y - 12));
  ...
  for (const index of labelIndexes) {
    ...
    ctx.fillText(formatChartAge(point.at, now), point.x, y0 + 24);
  }
  ...
  ctx.textAlign = 'left';
  ctx.font = '700 13px Arial';
  drawChartPill(ctx, 54, 410, 'Base ' + formatChartValue(baseValue), '#172235', '#2b3b59', '#dce6f6');
  drawChartPill(ctx, 184, 410, 'High ' + formatChartValue(high), '#172235', '#2b3b59', '#dce6f6');
  drawChartPill(ctx, 314, 410, 'Low ' + formatChartValue(low), '#172235', '#2b3b59', '#dce6f6');
  const trendText = (delta > 0 ? '+' : '') + formatChartValue(delta) + ' (' + (percent > 0 ? '+' : '') + percent.toFixed(1) + '%)';
  drawChartPill(ctx, 444, 410, 'Trend ' + trendText, delta > 0 ? '#12342d' : delta < 0 ? '#351d29' : '#172235', delta > 0 ? '#2f705f' : delta < 0 ? '#70425b' : '#2b3b59', '#dce6f6');
...
if (!globalThis.__fishyMarketValueChartPolishPatch) {
  ...
  Module._extensions['.js'] = function polishedFishyMarketLoader(module, filename) {
    if (!isFishyMarketFile(filename)) return previousLoader(module, filename);

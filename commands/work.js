...
function ensureStore() {
  ...
  if (!fs.existsSync(STORE_PATH)) fs.writeFileSync(STORE_PATH, JSON.stringify({ users: {} }, null, 2), 'utf8');
}
function loadState() {
  ensureStore();
  try {
...
function getUserRecord(state, userId) {
  if (!state.users[userId]) state.users[userId] = { jobId: null, totalWorks: 0, daily: {} };
  const user = state.users[userId];
  user.jobId = typeof user.jobId === 'string' ? user.jobId : null;
  user.totalWorks = Math.max(0, Math.floor(Number(user.totalWorks) || 0));
  user.daily = user.daily && typeof user.daily === 'object' ? user.daily : {};
  return user;
}
...
function recordWork(userId) {
  ...
  user.totalWorks += 1;
  user.daily[key] = Math.max(0, Math.floor(Number(user.daily[key]) || 0)) + 1;
  saveState(state);
}
...
function eligibility(interaction, job) {
  const p = profile(interaction.user.id);
  return { missingWorks: Math.max(0, job.requiredWorks - p.totalWorks), missingLevel: Math.max(0, job.requiredLevel - userLevel(interaction)) };
}
...
function home(interaction, notice = '') {
  ...
  const cd = getWorkCooldown(interaction.user.id);
...
function requirementText(interaction, job) {
  const e = eligibility(interaction, job);
  const parts = [];
...
function jobsPage(interaction, page = 0) {
  const p = profile(interaction.user.id);
  const maxPage = Math.max(1, Math.ceil(JOBS.length / JOBS_PER_PAGE));
  const safePage = ((Math.floor(Number(page) || 0) % maxPage) + maxPage) % maxPage;
  ...
  for (const job of JOBS.slice(safePage * JOBS_PER_PAGE, (safePage * JOBS_PER_PAGE) + JOBS_PER_PAGE)) {
    const applied = p.jobId === job.id;
    const ok = canApply(interaction, job);
...
function header(interaction, session, line) {
  return [`## ${interaction.user} are you ready to work?`, `-# You have <t:${Math.floor(session.expiresAt / 1000)}:R> to complete.`, `-# Job: ${session.job.name} // ${range(session.job)}`, line].join('\n');
}
...
function memoryGuessPayload(interaction, session) {
  const done = new Set(session.doneOptions || []);
  return payload([text(header(interaction, session, 'What was the pattern in order?')), sep(), row(...session.options.map((emoji, i) => button(`work:memory:${interaction.user.id}:${session.id}:${i}`, emoji, done.has(i) ? 3 : 2, done.has(i))))]);
}
...
async function showFinished(interaction, session, ok, customText = '') {
  cleanup(session.id);
  ...
  const line = ok ? `* ${YES} You have completed your work and earned ${formatAbbreviated(earned)} ${PRCOIN}` : `* ${NO} ${customText || `You have failed your work and only earned ${formatAbbreviated(earned)} ${PRCOIN}`}`;
  ...
  await session.message?.edit(h).catch(() => null);
  if (!interaction.replied && !interaction.deferred && interaction.isRepliable?.()) await interaction.reply({ content: 'Work submitted.', flags: EPHEMERAL_FLAG }).catch(() => null);
}
function timeoutFail(interaction, session, ms) {
  session.timer = setTimeout(async () => {
    const current = activeSessions.get(session.id);
    if (!current || current.finished) return;
    current.finished = true;
    const base = ri(current.job.min, current.job.max);
    const earned = Math.max(1, Math.floor(base * 0.10));
    addBalance(interaction.user.id, earned);
    recordGamblingEarnings(interaction.user.id, earned);
    recordWork(interaction.user.id);
    setWorkCooldown(interaction.user.id, Date.now() + WORK_COOLDOWN_MS);
    cleanup(current.id);
    await current.message?.edit(home(interaction, `* ${NO} You have failed your work and only earned ${formatAbbreviated(earned)} ${PRCOIN}`)).catch(() => null);
  }, ms);
}
...
async function startMemory(interaction, job) {
  const pattern = Array.from({ length: 4 }, () => pick(MEMORY_EMOJIS));
  const s = { id: sid(), userId: interaction.user.id, job, pattern, options: shuffle(pattern), doneOptions: [], next: 0, expiresAt: Date.now() + 20000, message: interaction.message };
  activeSessions.set(s.id, s);
  await interaction.update(memoryRevealPayload(interaction, s));
  s.revealTimer = setTimeout(async () => { const current = activeSessions.get(s.id); if (!current || current.finished) return; current.expiresAt = Date.now() + 15000; timeoutFail(interaction, current, 15000); await interaction.message?.edit(memoryGuessPayload(interaction, current)).catch(() => null); }, ri(3000, 5000));
}
async function startWork(interaction) {
  const job = getJob(profile(interaction.user.id).jobId);
  if (!job || getWorkCooldown(interaction.user.id) > Date.now()) return interaction.update(home(interaction)).catch(() => null);
  const game = pick(job.games);
  if (game === 'typing') return startTyping(interaction, job);
  if (game === 'math') return startMath(interaction, job);
  return startMemory(interaction, job);
}
function showModal(interaction, id, title, label, placeholder) {
  const modal = new ModalBuilder().setCustomId(id).setTitle(title).addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('answer').setLabel(label).setStyle(TextInputStyle.Short).setRequired(true).setMinLength(1).setMaxLength(40).setPlaceholder(placeholder || '')));
...
module.exports = {
  ...
  async handleInteraction(interaction) {
    if (interaction.isButton?.() && interaction.customId?.startsWith('work:')) {
      ...
      if (action === 'memory') {
        const s = activeSessions.get(parts[3]); const optionIndex = Number(parts[4]);
        if (!s || s.userId !== interaction.user.id) { await interaction.reply({ content: 'This memory challenge is no longer active.', flags: EPHEMERAL_FLAG }); return true; }
        if (s.options[optionIndex] !== s.pattern[s.next]) { s.finished = true; await showFinished(interaction, s, false); return true; }
        s.doneOptions.push(optionIndex); s.next += 1;
        if (s.next >= s.pattern.length) { s.finished = true; await showFinished(interaction, s, true); return true; }
        await interaction.update(memoryGuessPayload(interaction, s));
        return true;
      }
    }
    if (interaction.isModalSubmit?.() && interaction.customId?.startsWith('workmodal:')) {
      const parts = interaction.customId.split(':');
      if (parts[2] !== interaction.user.id) { await interaction.reply({ content: 'You can only submit your own work challenge.', flags: EPHEMERAL_FLAG }); return true; }
      const s = activeSessions.get(parts[3]);
      if (!s) { await interaction.reply({ content: 'This work challenge is no longer active.', flags: EPHEMERAL_FLAG }); return true; }
      const answer = interaction.fields.getTextInputValue('answer').trim();
      s.finished = true;
      const ok = parts[1] === 'typing' ? answer === s.code : answer.replace(/,/g, '') === s.answer;
      await showFinished(interaction, s, ok);
      return true;
    }
    return false;
  },

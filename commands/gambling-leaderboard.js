...
function parseState(customId) {
  ...
  return { ownerId: parts[2], type: parts[3] || 'money', difficulty: parts[4] || 'all', moneyMode: parts[5] || MONEY_MODES.EARNED };
}
function normalizeDifficulty(type, difficulty) {
  const options = getDifficultyOptions(type);
  if (!options.length) return 'all';
  return options.includes(difficulty) ? difficulty : options[0];
}
function buildControls(ownerId, type, selectedDifficulty, selectedMoneyMode) {
  ...
  rows.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    .addOptions(TYPE_OPTIONS.map((option) => ({ label: option.label, value: option.value, description: option.description, default: option.value === type })))).toJSON());
  if (type === 'money') {
  ...
  const options = getDifficultyOptions(type);
  if (options.length) rows.push(new ActionRowBuilder().addComponents(options.map((difficulty) => new ButtonBuilder().setCustomId(`gamblinglb:filter:${ownerId}:${type}:${difficulty}:${selectedMoneyMode}`).setLabel(difficultyLabel(type, difficulty)).setStyle(difficulty === selectedDifficulty ? ButtonStyle.Success : ButtonStyle.Secondary).setDisabled(difficulty === selectedDifficulty))).toJSON());
  return rows;
}
...
function buildPayload(guild, ownerId, type, difficulty, moneyMode, attachment) {
  ...
  return { flags: COMPONENTS_V2_FLAG, files: [attachment], components: [{ type: 17, accent_color: 0xffffff, components: [
    { type: 10, content: [`## ${guild.name}'s gambling leaderboard.`, `-# Category: **${getTitle(type, difficulty, moneyMode)}**`, `-# Refresh: <t:${nextUpdateUnix}:R> (<t:${nextUpdateUnix}:t> UTC+7)`].join('\n') },
    { type: 12, items: [{ media: { url: 'attachment://gambling-leaderboard.png' } }] },
...
async function sendLeaderboard(target, guild, ownerId, type = 'money', difficulty = 'all', moneyMode = MONEY_MODES.EARNED, mode = 'reply') {
  ...
  const rows = [...allUserIds].map((userId) => ({ userId, score: getScore(type, difficulty, allStats[userId] || {}, allBalances[userId] || 0, moneyMode) })).filter((entry) => entry.score > 0).sort((a, b) => b.score - a.score).slice(0, 10).map((entry, index) => ({ ...entry, rank: index + 1 }));
  const rowsWithMeta = await Promise.all(rows.map(async (row) => {
    ...
    return { ...row, username: member?.user?.username || `Unknown (${row.userId})`, avatarUrl: member?.user?.displayAvatarURL({ extension: 'png', size: 128 }) || '', displayValue: formatCompactNumber(row.score) };
  }));
...
async function refreshTrackedLeaderboards() {
  ...
  for (const [messageId, state] of activeLeaderboardMessages.entries()) {
    const guild = schedulerClient.guilds.cache.get(state.guildId) || await schedulerClient.guilds.fetch(state.guildId).catch(() => null);
    if (!guild) continue;
    ...
    if (!message) { activeLeaderboardMessages.delete(messageId); continue; }
...
function scheduleLeaderboardRefresh() {
  ...
  const delay = Math.max(1_000, getNextHourlyBoundaryUtcPlus7().getTime() - Date.now());
  leaderboardScheduler = setTimeout(async () => { await refreshTrackedLeaderboards().catch(() => null); scheduleLeaderboardRefresh(); }, delay);
}
function rememberLeaderboardMessage(message, state) {
  if (!message?.id || !message?.channelId || !message?.guildId) return;
  activeLeaderboardMessages.set(message.id, { ownerId: state.ownerId, guildId: message.guildId, channelId: message.channelId, type: state.type, difficulty: state.difficulty, moneyMode: state.moneyMode });
}
async function deferForLeaderboard(interaction) {
  if (interaction.isChatInputCommand?.() && !interaction.deferred && !interaction.replied) { await interaction.deferReply().catch(() => null); return 'edit'; }
  if ((interaction.isButton?.() || interaction.isStringSelectMenu?.()) && !interaction.deferred && !interaction.replied) { await interaction.deferUpdate().catch(() => null); return 'edit'; }
  if (interaction.isButton?.() || interaction.isStringSelectMenu?.()) return 'update';
  return 'reply';
}

module.exports = {
  data: new SlashCommandBuilder().setName('gambling-leaderboard').setDescription('Show gambling leaderboard'),
  async init(client) { schedulerClient = client; scheduleLeaderboardRefresh(); },
  async execute(interaction) {
    ...
    rememberLeaderboardMessage(message, { ownerId: interaction.user.id, type: 'money', difficulty: 'all', moneyMode: MONEY_MODES.EARNED });
  },
  async handleInteraction(interaction) {
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('gamblinglb:type:')) {
      ...
      if (ownerId !== interaction.user.id) { await interaction.reply({ content: 'You can only use controls from your own gambling leaderboard command.', flags: MessageFlags.Ephemeral }); return true; }
      const type = interaction.values?.[0] || 'money';
      const fallbackDifficulty = normalizeDifficulty(type, difficulty);
      const mode = await deferForLeaderboard(interaction);
      await sendLeaderboard(interaction, interaction.guild, ownerId, type, fallbackDifficulty, moneyMode, mode);
      rememberLeaderboardMessage(interaction.message || await interaction.fetchReply().catch(() => null), { ownerId, type, difficulty: fallbackDifficulty, moneyMode });
      return true;
    }
    if (interaction.isButton() && interaction.customId.startsWith('gamblinglb:filter:')) {
      ...
      rememberLeaderboardMessage(interaction.message || await interaction.fetchReply().catch(() => null), { ownerId, type, difficulty, moneyMode });
      return true;
    }

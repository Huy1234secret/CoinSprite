...
test('global commands stay empty and enabled guilds always include game commands', () => {
  ...
  assert.deepEqual(featureCommandsForConfig(base).map((command) => command.name), ['cs-balance', 'cs-work', 'cs-beg', 'cs-inventory', 'cs-shop', 'cs-trivia', 'cs-achievements']);
  assert.deepEqual(featureCommandsForConfig({
    ...base,
    features: { ...base.features, leveling: true },
    leveling: { enabled: true },
  }).map((command) => command.name), ['cs-balance', 'cs-work', 'cs-beg', 'cs-inventory', 'cs-shop', 'cs-trivia', 'cs-achievements', 'level', 'leaderboard', 'level-set', 'xp-add', 'leveling-setup', 'drop-crate']);
});

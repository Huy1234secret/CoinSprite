...
test('dashboard exposes controls, accessible gear settings, JSON round-trip, and resolved preview hooks', () => {
  ...
  const css = fs.readFileSync(path.join(__dirname, '..', 'admin', 'workspace.css'), 'utf8');
  ...
  assert.match(app, /A dropdown must keep at least one option/);
  assert.match(app, /confirmAction\(\{[\s\S]*Delete “/);
  assert.match(app, /templateOptionValidation/);
  assert.match(app, /Use a unique title in this dropdown/);
  assert.match(app, /templateControlAt\(target\.dataset\.templateControlAction\)/);
  ...
  assert.match(css, /\.template-control-row[^}]*flex-wrap:\s*wrap/);
  assert.match(css, /\.template-dropdown-settings \.rr-allow-multiple > span/);
  assert.match(css, /container-type:\s*inline-size/);
  assert.match(css, /@container \(max-width:\s*420px\)/);
  assert.match(css, /\.template-control-row > \*[^}]*min-width:\s*0/);
  assert.match(css, /\.template-dropdown-card[^}]*max-width:\s*100%/);
  assert.match(css, /html\s*\{[^}]*min-width:\s*0/);
  assert.doesNotMatch(css, /\.template-control-row\.dropdown\s*\{[^}]*minmax\(1[1-9][0-9]px/);

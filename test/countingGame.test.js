...
test('game commands are registered for every enabled guild even when no Counting channel is set', () => {
  ...
  assert.deepEqual(commands.map((command) => command.name), ['cs-balance', 'cs-work', 'cs-beg', 'cs-inventory', 'cs-shop', 'cs-trivia', 'cs-achievements']);
});
...
test('dashboard exposes Games/Counting and PATCH channel validation is guild isolated', async () => {
  ...
  assert.match(html, /data-view="games"[^>]*>[\s\S]*?<strong>Games routing<\/strong>/);
  assert.match(html, /class="game-catalog"[\s\S]*Counting/);

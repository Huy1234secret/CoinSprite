...
test('Luck Tier 0 produces identical base and current marginal crop probabilities', () => {
  ...
  assert.deepEqual(starFruit.currentChance, starFruit.baseChance);
...
test('profile masking excludes secrets and hides every undiscovered crop field', () => {
  const source = repository({
    luckTier: 5,
    discoveries: ['eclipse_bloom', 'star_fruit'],
  });
...
test('discovered visible crops expose only their display comparison fields', () => {
  ...
  assert.match(crop.currentChance.percentage, /%$/);

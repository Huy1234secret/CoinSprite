...
test('Work progression, inventory, active-session lock, and cooldown are global but isolated by user', () => {
  ...
  try {
    ...
    assert.deepEqual(settlement.profile, {
      userId: USER_A, level: 2, xp: 15, streak: 8, cooldownUntil: now + WORK_COOLDOWN_MS, careerId: null, career: null, dayStart: 0, dailyCompleted: 0, dailyBoostTier: 0, salaryBoost: 0, jobChangeUntil: 0, totalCompleted: 1, salaryBoostBonus: 5,
    });
    assert.equal(work.balance(USER_A), 25n);
    ...
    assert.deepEqual(work.profile(USER_B), {
      userId: USER_B, level: 1, xp: 0, streak: 0, cooldownUntil: 0, careerId: null, career: null, dayStart: 0, dailyCompleted: 0, dailyBoostTier: 0, salaryBoost: 0, jobChangeUntil: 0, totalCompleted: 0, salaryBoostBonus: 0,
    });

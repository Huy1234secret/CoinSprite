...
function getDefaultXpRule(xpConfig) {
  const minXp = Math.max(0, Number(xpConfig.messageXpMin) || 0);
  return {
    minXp,
    maxXp: Math.max(minXp, Number(xpConfig.messageXpMax) || minXp),
...
function getXpChannelOverrides(guildId) {
  ...
  return (source || [])
    .map((raw) => ({ raw, rule: normalizeXpChannelRule(raw, xpConfig) }))
    .filter(({ raw, rule }) => rule && (hasExplicitOverrides || (typeof raw !== 'string' && !isDefaultRule(rule, xpConfig))))
    .map(({ rule }) => rule);
}

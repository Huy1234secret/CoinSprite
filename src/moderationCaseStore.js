...
function readState() {
  const raw = readJsonFile(STORE_PATH, { fallback: emptyState, label: 'Moderation case store' });
  const sourceVersion = Math.max(1, Number(raw?.version) || 1);
  const state = normalizeState(raw);
  if (sourceVersion < VERSION) {
    backupFileOnce(STORE_PATH, migrationBackupPath(sourceVersion));
    writeState(state);
    console.info('Migrated moderation case store from schema v' + sourceVersion + ' to v' + VERSION + '.');
  }
  return state;
}
...
function queryCases(guildId, filters = {}) {
  const pageSize = Math.max(1, Math.min(100, Math.round(Number(filters.pageSize) || 25)));
  const requestedPage = Math.max(1, Math.round(Number(filters.page) || 1));
  const all = listCases(guildId, filters);
  const total = all.length;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const page = Math.min(requestedPage, totalPages);
  const start = (page - 1) * pageSize;
  return {
    cases: all.slice(start, start + pageSize),

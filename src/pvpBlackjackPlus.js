...
function makeDeck() {
  ...
  for (const suit of SUITS) {
    for (const rank of RANKS) deck.push({ ...rank, suit: suit.symbol, suitDisplay: suit.display, color: suit.color });
  }
...
function cardShort(card) {
  return `${card.label} ${card.suitDisplay}`;
}

function handLine(hand) {
  return hand.map(cardShort).join(', ');
}
...
function payload(accent, components, files = []) {
  const data = { flags: COMPONENTS_V2_FLAG, components: [{ type: 17, accent_color: accent, components }] };
...
function drawHeart(ctx, cx, cy, size, color) {
  ...
  ctx.beginPath();
  ctx.moveTo(0, 34);
  ctx.bezierCurveTo(-55, -8, -36, -54, 0, -30);
  ctx.bezierCurveTo(36, -54, 55, -8, 0, 34);
  ctx.closePath();
  ctx.fillStyle = color;
  ctx.fill();
  ctx.restore();
...
function drawClub(ctx, cx, cy, size, color) {
  const r = size * 0.18;
  ctx.fillStyle = color;
  ctx.beginPath(); ctx.arc(cx, cy - size * 0.18, r, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx - size * 0.2, cy + size * 0.04, r, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + size * 0.2, cy + size * 0.04, r, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath();
  ctx.moveTo(cx - size * 0.08, cy + size * 0.15);
  ctx.lineTo(cx + size * 0.08, cy + size * 0.15);
  ctx.lineTo(cx + size * 0.16, cy + size * 0.43);
  ctx.lineTo(cx - size * 0.16, cy + size * 0.43);
  ctx.closePath();
  ctx.fill();
}
...
function drawSpade(ctx, cx, cy, size, color) {
  ...
  ctx.translate(cx, cy);
  ctx.rotate(Math.PI);
  drawHeart(ctx, 0, 0, size, color);
  ctx.restore();
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.moveTo(cx - size * 0.09, cy + size * 0.12);
  ctx.lineTo(cx + size * 0.09, cy + size * 0.12);
...
function drawSuitImage(ctx, card, cx, cy, size) {
  if (card.suit === 'H') drawHeart(ctx, cx, cy, size, card.color);
  else if (card.suit === 'D') drawDiamond(ctx, cx, cy, size, card.color);
  else if (card.suit === 'C') drawClub(ctx, cx, cy, size, card.color);
  else drawSpade(ctx, cx, cy, size, card.color);
}
...
function drawCardFace(ctx, card, x, y, w, h) {
  ...
  ctx.font = card.label === '10' ? 'bold 28px sans-serif' : 'bold 34px sans-serif';
  ctx.fillText(card.label, x + 13, y + 11);
  drawSuitImage(ctx, card, x + w / 2, y + h / 2 + 12, 76);
}

function drawHand(ctx, hand, x, y, reveal, maxWidth = 390) {
  const cardW = 92;
  const cardH = 132;
  const overlap = hand.length > 5 ? Math.max(42, Math.floor((maxWidth - cardW) / Math.max(1, hand.length - 1))) : 70;
  for (let i = 0; i < hand.length; i += 1) {
    const cx = x + i * overlap;
    if (reveal) drawCardFace(ctx, hand[i], cx, y, cardW, cardH);
    else drawHiddenCard(ctx, cx, y, cardW, cardH);
  }
}
...
function tableAttachment(game) {
  ...
  ctx.fillText('PVP BLACKJACK', width / 2, 58);
  ctx.font = 'bold 18px sans-serif';
  ctx.fillStyle = '#f6f1c9';
  ctx.fillText(`${formatNumber(game.bet)} PRcoin each`, width / 2, 84);
  ...
  ctx.fillText(reveal ? `Score: ${handValue(left.hand)}` : `${left.hand.length} hidden cards`, 76, 158);
  drawHand(ctx, left.hand, 76, 186, reveal, 390);
  ...
  ctx.font = '18px sans-serif';
  ctx.fillStyle = '#d7dce2';
  ctx.fillText(reveal ? `Score: ${handValue(right.hand)}` : `${right.hand.length} hidden cards`, 924, 158);
  drawHand(ctx, right.hand, 534, 186, reveal, 390);

  ctx.textAlign = 'center';
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 24px sans-serif';
  ctx.fillText(reveal ? (game.summaryText || 'Game finished') : `${current?.username || 'Player'} is deciding...`, width / 2, 510);

  return new AttachmentBuilder(canvas.toBuffer('image/png'), { name: 'pvp-blackjack-table.png' });
}

function ansiYellow(content) {
...
function buildChallengePayload(challenge, state = 'pending') {
  ...
  const lines = [
    '## PVP Blackjack Challenge',
    state === 'pending' ? `${other} wants to play Blackjack against ${asker}.` : challenge.summary || 'Challenge ended.',
    `-# Current bet: ${formatNumber(challenge.amount)} ${PRCOIN} each`,
  ];
  if (state === 'pending') lines.push(`-# ${asker}, choose Yes, No, or Higher Bet. Max bet: ${formatNumber(MAX_BET)} ${PRCOIN}.`);
  ...
  return payload(state === 'pending' ? BLUE_ACCENT : RED_ACCENT, components);
}

function buildGamePayload(game) {
  const reveal = game.status === 'finished';
  const attachment = tableAttachment(game);
  const lines = [
    '## PVP Blackjack',
  ...
  if (reveal) {
    for (const player of game.players) {
      lines.push(`-# ${player.mention}: ${handLine(player.hand)} (${handValue(player.hand)})${player.busted ? ' — BUST' : ''}`);
    }
    lines.push(game.summary || 'Game finished.');
    if (game.winAmount > 0) lines.push(ansiYellow(`Won: ${formatNumber(game.winAmount)} PRcoin`));
  } else {
    const current = game.players[game.turnIndex];
    lines.push(`### ${current.mention}'s turn`);
    lines.push('-# Cards stay hidden on the table. Use Show Cards to privately view your hand and score.');
    for (const player of game.players) {
      const status = player.stayed ? (player.busted ? 'busted' : 'stayed') : `${player.hand.length} hidden cards`;
      lines.push(`-# ${player.mention}: ${status}`);
    }
  }
  const components = [text(lines.join('\n')), gallery('pvp-blackjack-table.png'), separator()];
  if (reveal) components.push(row(button(`${PREFIX}:done:${game.id}`, 'Game Over', 2, true)));
  else {
    components.push(row(
      button(`${PREFIX}:hit:${game.id}`, 'Hit', 1),
      button(`${PREFIX}:stay:${game.id}`, 'Stay', 2),
      button(`${PREFIX}:show:${game.id}`, 'Show Cards', 2),
    ));
  }
  let accent = BLUE_ACCENT;
  if (game.status === 'finished' && game.outcome === 'push') accent = YELLOW_ACCENT;
  if (game.status === 'finished' && game.outcome === 'win') accent = GREEN_ACCENT;
  return payload(accent, components, [attachment]);
}
...
function resetGameTimer(game) {
  ...
  game.timer = setTimeout(async () => {
    ...
    currentPlayer(game).stayed = true;
...
function settleGame(game) {
  ...
  if (winner) {
    ...
    addBalance(winner.id, pot);
    recordGamblingEarnings(winner.id, pot);
    game.outcome = 'win';
    game.winAmount = pot;
    game.summary = `${winner.mention} wins the pot.`;
    game.summaryText = `${winner.username} wins ${formatNumber(pot)} PRcoin`;
  } else {
    for (const player of game.players) addBalance(player.id, game.bet);
    game.outcome = 'push';
    game.winAmount = 0;
    game.summary = 'Push! Both bets were returned.';
    game.summaryText = 'Push — bets returned';
  }
  cleanupGame(game);
}

function advanceOrFinish(game) {
  if (!advanceTurn(game)) settleGame(game);
  else resetGameTimer(game);
}
...
async function startPvpChallenge(interaction) {
  ...
  const blockReason = getCommandBlockReason(challenger.id, 'pvp-blackjack');
  if (blockReason) {
  ...
  if (isBusy(challenger.id)) {
    await interaction.reply({ content: 'You already have an active PVP Blackjack game or challenge.', flags: EPHEMERAL_FLAG });
    return;
  }
  if (isBusy(target.id)) {
    await interaction.reply({ content: `${target} already has an active PVP Blackjack game or challenge.`, flags: EPHEMERAL_FLAG });
    return;
  }
  ...
  markBusy(challenger.id, 'challenge', challenge.id);
  markBusy(target.id, 'challenge', challenge.id);
...
async function beginGame(challenge, interaction) {
  ...
  const game = {
    ...
    deck,
    players: [
      { id: challenge.challengerId, username: challenge.challengerName, mention: `<@${challenge.challengerId}>`, hand: [drawCard(deck), drawCard(deck)], stayed: false, busted: false },
      { id: challenge.targetId, username: challenge.targetName, mention: `<@${challenge.targetId}>`, hand: [drawCard(deck), drawCard(deck)], stayed: false, busted: false },
    ],
    turnIndex: 0,
    status: 'active',
    outcome: null,
...
async function handlePvpInteraction(interaction) {
  ...
  if (interaction.isModalSubmit?.()) {
    ...
    if (!challenge) {
      await interaction.reply({ content: 'This challenge is no longer active.', flags: EPHEMERAL_FLAG });
      return true;
    }
        ...
        components: [
          {
            ...
            component: {
              ...
              value: getLastBetInput(interaction.user.id, 'pvp-blackjack') || String(Math.min(MAX_BET, challenge.amount + 1)),
            },
          },
        ],
      });
  ...
  if (!player) {
    await interaction.reply({ content: 'Only players in this match can use these buttons.', flags: EPHEMERAL_FLAG });
    return true;
  }

  if (action === 'show') {
    await interaction.reply({
      content: [`Your cards: ${handLine(player.hand)}`, `Total points: ${handValue(player.hand)}`].join('\n'),
...
function shouldLogPvpBlackjackInteraction(interaction) {

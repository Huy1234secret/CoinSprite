...
function timeoutGame(game) { const inactive = currentPlayer(game); const winner = game.players.find((player) => player.id !== inactive.id); const pot = game.bet * 2; game.status = 'finished'; game.outcome = 'timeout'; game.winAmount = pot; if (winner) { addBalance(winner.id, pot); recordGamblingEarnings(winner.id, pot); game.summary = `${inactive.mention} was inactive for 30 seconds. ${winner.mention} takes the pot.`; game.summaryText = `${winner.username} wins ${formatNumber(pot)} PRcoin by inactivity`; } else { game.summary = 'The game ended by inactivity.'; game.summaryText = 'Ended by inactivity'; } cleanupGame(game); }
function resetGameTimer(game) { clearTimer(game); game.timer = setTimeout(async () => { if (!activeGames.has(game.id) || game.status !== 'active') return; timeoutGame(game); await game.message?.edit(buildGamePayload(game)).catch(() => null); }, TURN_TIMEOUT_MS); }
...
async function handlePvpInteraction(interaction) {
  ...
  if (['accept', 'decline', 'higher'].includes(action)) {
    ...
    if (action === 'higher') {
      ...
      await interaction.showModal({
        ...
        components: [{ type: 18, label: `New bet, max ${formatNumber(MAX_BET)} PRcoin`, component: { type: 4, custom_id: 'amount', style: 1, required: true, min_length: 1, max_length: 12, placeholder: 'Example: 5000, 5k, all', value: getLastBetInput(interaction.user.id, 'pvp-blackjack') || String(Math.min(MAX_BET, challenge.amount + 1)) } }],
      });
      return true;
    }
  ...
  if (action === 'show') {
    if (interaction.user.id === current.id) resetGameTimer(game);
    await interaction.reply({ content: [`Your cards: ${handLine(player.hand)}`, `Total points: ${handValue(player.hand)}`].join('\n'), flags: EPHEMERAL_FLAG });
    return true;
  }

...
- Duplicate/stale suppression is intentionally silent in the owner console. A destination with missing Discord permissions is paused for that stock cycle without triggering a global retry of every other server, and an unchanged permission problem is reported only once.

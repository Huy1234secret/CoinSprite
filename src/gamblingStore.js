...
function getIncomeClaim(userId) { const state = loadState(); const entry = state.incomeClaims[userId] && typeof state.incomeClaims[userId] === 'object' ? state.incomeClaims[userId] : {}; return { startedAt: Math.max(0, Math.floor(Number(entry.startedAt) || 0)), lastClaimAt: Math.max(0, Math.floor(Number(entry.lastClaimAt) || 0)) }; }

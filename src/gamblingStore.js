...
function setBalance(userId, amount) { const state = loadState(); state.balances[userId] = Math.max(0, Math.floor(Number(amount) || 0)); saveState(state); return state.balances[userId]; }
function addBalance(userId, amount) { const state = loadState(); const next = Math.max(0, Math.floor(Number(state.balances[userId] || 0) + amount)); state.balances[userId] = next; saveState(state); return next; }
function spendBalance(userId, amount) { const state = loadState(); const spend = Math.floor(Number(amount) || 0); const current = Number(state.balances[userId] || 0); if (!Number.isFinite(spend) || spend <= 0 || current < spend) return false; state.balances[userId] = current - spend; saveState(state); return true; }
...
function getUserStatsRecord(state, userId) {
  ...
  state.stats[userId] = {
    ...base,
    ...current,
    triviaBestRun: { ...base.triviaBestRun, ...(current.triviaBestRun || {}) },
    minefieldCompleted: { ...base.minefieldCompleted, ...(current.minefieldCompleted || {}) },
    rouletteStraightWins: { ...base.rouletteStraightWins, ...(current.rouletteStraightWins || {}) },
  };
  return state.stats[userId];
}

function getJackpotBalance(userId) { return Number(loadState().jackpotBalances[userId] || 0); }
function addJackpotBalance(userId, amount) {
  const state = loadState();
  const delta = Math.max(0, Math.floor(Number(amount) || 0));
  const next = Math.max(0, Math.floor(Number(state.jackpotBalances[userId] || 0) + delta));
  state.jackpotBalances[userId] = next;
  if (delta > 0) {
    const stats = getUserStatsRecord(state, userId);
    stats.rouletteStraightWins.straight = Math.max(0, Math.floor(Number(stats.rouletteStraightWins.straight) || 0) + delta);
  }
  saveState(state);
  return next;
}
function spendJackpotBalance(userId, amount) { const state = loadState(); const spend = Math.floor(Number(amount) || 0); const current = Number(state.jackpotBalances[userId] || 0); if (!Number.isFinite(spend) || spend <= 0 || current < spend) return false; state.jackpotBalances[userId] = current - spend; saveState(state); return true; }
function resetAllGamblingData() { const emptyState = getEmptyState(); saveState(emptyState); return emptyState; }

function recordGamblingEarnings(userId, amount) { const delta = Math.max(0, Math.floor(Number(amount) || 0)); if (delta <= 0) return 0; const state = loadState(); const stats = getUserStatsRecord(state, userId); stats.moneyEarned = Math.max(0, Math.floor(Number(stats.moneyEarned) || 0) + delta); saveState(state); return stats.moneyEarned; }
function recordRouletteStraightWin(userId, amount = 1) { const delta = Math.max(0, Math.floor(Number(amount) || 0)); if (delta <= 0) return 0; const state = loadState(); const stats = getUserStatsRecord(state, userId); stats.rouletteStraightWins.straight = Math.max(0, Math.floor(Number(stats.rouletteStraightWins.straight) || 0) + delta); saveState(state); return stats.rouletteStraightWins.straight; }
function recordTriviaRun(userId, perDifficultyCorrect = {}, isRandomRun = false) { const easy = Math.max(0, Math.floor(Number(perDifficultyCorrect.easy) || 0)); const medium = Math.max(0, Math.floor(Number(perDifficultyCorrect.medium) || 0)); const hard = Math.max(0, Math.floor(Number(perDifficultyCorrect.hard) || 0)); const all = isRandomRun ? (easy + medium + hard) : 0; const state = loadState(); const stats = getUserStatsRecord(state, userId); stats.triviaCorrectTotal = Math.max(0, Math.floor(Number(stats.triviaCorrectTotal) || 0)) + all; stats.triviaBestRun.easy = Math.max(stats.triviaBestRun.easy, easy); stats.triviaBestRun.medium = Math.max(stats.triviaBestRun.medium, medium); stats.triviaBestRun.hard = Math.max(stats.triviaBestRun.hard, hard); stats.triviaBestRun.all = Math.max(stats.triviaBestRun.all, all); saveState(state); return stats.triviaBestRun; }
function getTriviaXpMultiplier(userId) { const state = loadState(); const stats = getUserStatsRecord(state, userId); const totalCorrect = Math.max(0, Math.floor(Number(stats.triviaCorrectTotal) || 0)); return Number((1.1 ** Math.floor(totalCorrect / 50)).toFixed(6)); }
...
function getAllBalances() { return JSON.parse(JSON.stringify(loadState().balances || {})); }
...
function getIncomeClaim(userId) {
  const state = loadState();
  const entry = state.incomeClaims[userId] && typeof state.incomeClaims[userId] === 'object' ? state.incomeClaims[userId] : {};
  return {
    startedAt: Math.max(0, Math.floor(Number(entry.startedAt) || 0)),
    lastClaimAt: Math.max(0, Math.floor(Number(entry.lastClaimAt) || 0)),
  };
}
function setIncomeClaim(userId, values = {}) {
  const state = loadState();
  if (!state.incomeClaims[userId] || typeof state.incomeClaims[userId] !== 'object') state.incomeClaims[userId] = {};
  if (values.startedAt !== undefined) state.incomeClaims[userId].startedAt = Math.max(0, Math.floor(Number(values.startedAt) || 0));
  if (values.lastClaimAt !== undefined) state.incomeClaims[userId].lastClaimAt = Math.max(0, Math.floor(Number(values.lastClaimAt) || 0));
  saveState(state);
  return getIncomeClaim(userId);
}

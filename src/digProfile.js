...
const DIG_DATA_FILE = path.join(__dirname, '..', 'data', 'dig_profiles.json');
...
const DEFAULT_PROFILE = {
  ...
  dig_upgrade_tokens_used: 0,
  upgrade_tokens: 0,
};

function loadProfiles() {
  if (!fs.existsSync(DIG_DATA_FILE)) {
    return {};
  }

  try {
    const raw = fs.readFileSync(DIG_DATA_FILE, 'utf-8');
    const data = JSON.parse(raw);
    return typeof data === 'object' && data !== null ? data : {};
  } catch (error) {
    console.warn('Failed to read dig profiles; starting fresh.', error);
    return {};
  }
}

function saveProfiles(profiles) {
  fs.mkdirSync(path.dirname(DIG_DATA_FILE), { recursive: true });
  fs.writeFileSync(DIG_DATA_FILE, JSON.stringify(profiles));
}
...
function ensureProfileShape(profile = {}) {
  ...
  const used = Math.max(0, Math.floor(Number(profile.dig_upgrade_tokens_used) || 0));
...
function getUserDigProfile(userId) {
...
function updateUserDigProfile(userId, profile) {
  const profiles = loadProfiles();
  profiles[String(userId)] = ensureUpgradeTokenBalance(ensureProfileShape(profile));
  saveProfiles(profiles);
}

function addDigXp(userId, amount) {
  const profiles = loadProfiles();
  const userKey = String(userId);

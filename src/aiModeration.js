...
const BATCH_SYSTEM_PROMPT = [
  'Moderate only the ten entries under Messages to check. The numbered Context entries are reference material only and must never be flagged.',
  'Use the preceding context to interpret intent, slang, replies, and deliberate bypass spelling. Detect leetspeak, phonetic misspellings, inserted punctuation or spaces, homoglyphs, and abbreviated slurs or sexual terms.',
  'For each target message, return exactly one result with the same 1-based index. If a target is not English, put its natural English translation in translated; otherwise translated must be empty.',
  'Return strict JSON. Flag only clear violations commonly enforced in most Discord communities. Use s=2-10, a concise case label, and a specific 1-3 sentence reason.',
].join(' ');

function batchModerationSchema() {
  return {
    type: 'object',
    additionalProperties: false,
    required: ['results'],
    properties: {
      results: {
        ...
        items: {
          type: 'object',
          additionalProperties: false,
          required: ['index', 'flagged', 's', 'case', 'reason', 'translated'],
          properties: {
...
function batchResponsesTextFormat() {
  return {
    type: 'json_schema',
    name: 'moderation_batch_result',
    strict: true,
    schema: batchModerationSchema(),
  };
}

function batchChatResponseFormat() {
  return {
    type: 'json_schema',
    json_schema: {
      name: 'moderation_batch_result',
...
function normalizeBatchResults(value, messages, source) {
  const rawResults = Array.isArray(value?.results) ? value.results : [];
  const byIndex = new Map();
  for (const raw of rawResults) {
    const index = Math.trunc(Number(raw?.index));
    if (index >= 1 && index <= messages.length && !byIndex.has(index)) byIndex.set(index, raw);
  }
  return messages.map((message, offset) => {
    const index = offset + 1;
    const raw = byIndex.get(index);
    const normalized = raw ? normalizeResult(raw, source) : cleanResult(source);
    return { ...normalized, batchIndex: index };
  });
}

function fallbackBatchResults(messages) {
  return messages.map((message, offset) => ({
...
async function analyzeModerationBatch(messages, context = {}) {
  const targets = (Array.isArray(messages) ? messages : []).slice(0, 10).map((value) => String(value || '').trim());
  if (targets.length !== 10) throw new Error('AI moderation batches require exactly 10 messages.');
  const fallback = fallbackBatchResults(targets);
  const apiKey = process.env.OPENAI_API_KEY;
  const input = String(context.batchInput || '').trim();
  if (!apiKey || typeof fetch !== 'function' || !input) return fallback;
  ...
  try {
    const payload = await postOpenAI(OPENAI_RESPONSES_API_URL, apiKey, {
      ...
      store: true,
      text: { format: batchResponsesTextFormat() },
    });
    try {
      recordUsage({ guildId: context.guildId, model, usage: payload.usage, source: 'openai-responses-batch' });
    } catch {}
    return normalizeBatchResults(parseJsonObject(responseText(payload)), targets, 'ai-batch');
  } catch (responsesError) {
    logAiModerationError('batch-responses-error', responsesError);
  }
  ...
  try {
    const payload = await postOpenAI(OPENAI_CHAT_API_URL, apiKey, {
      ...
      max_tokens: 2400,
      store: true,
      response_format: batchChatResponseFormat(),
    });
    try {
      recordUsage({ guildId: context.guildId, model, usage: payload.usage, source: 'openai-chat-batch' });
    } catch {}
    return normalizeBatchResults(parseJsonObject(chatText(payload)), targets, 'ai-chat-batch');
  } catch (chatError) {
    logAiModerationError('batch-chat-error', chatError);
    return fallback;
  }
}
...
module.exports = { analyzeModerationBatch, analyzeModerationMessage, fallbackAnalyze, shouldUseAiModeration };

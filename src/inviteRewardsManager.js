...
async function onMessageCreate(message) {
  ...
  if (!updateCmd) {
    ...
    await message.reply(
      'Invalid ! command. Use `!RI {userID}` (or `!IR {userID}`), `!DM {userID} {message} {yes/no}`, `!Message {channelID} {message} [replyto - messageID]`, `!Role add/remove [userID, userID] [roleID]`, `!add/remove {userID} {item} {amount}`, `!blacklist add/remove {userID} {reason}`, or `!invitee-blacklist add/remove {userID} {reason}`.',
    );
    return;
  }

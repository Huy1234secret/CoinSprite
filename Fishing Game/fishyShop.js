...
function rollStock(item) { const rules = Array.isArray(item.shopStock) ? item.shopStock : []; const total = rules.reduce((sum, rule) => sum + Math.max(0, Number(rule.chance) || 0), 0); if (total <= 0) return randomInt(1, 5); let roll = Math.random() * total; for (const rule of rules) { roll -= Math.max(0, Number(rule.chance) || 0); if (roll <= 0) return Math.max(0, Math.floor(Number(rule.amount) || 0)); } return Math.max(0, Math.floor(Number(rules[rules.length - 1]?.amount) || 0)); }

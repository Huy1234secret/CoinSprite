  ...
  const begin = async () => {
    ...
    try {
      ...
      updateUser(userId, (user) => {
        ...
        if (bait) consumeEquippedBait(user, bait);
        return user;
      });
...
async function handleReel(interaction, sessionId, buttonIndex) {
  ...
  if (session.remainingPower <= 0) {
    const variant = pickVariant(weatherNames);
